...
/// Stop the runtime for a WordPress/PHP project.
async fn dev_stop(project_path: &Path, force_playground: bool) -> AppResult {
    ...
    let pg = blueprint::PlaygroundRuntime::new();
    use blueprint::WordPressRuntime;
    if force_playground || pg.is_configured(project_path) {
        pg.stop(project_path)
            .map_err(|e| miette::miette!("{}", e))?;
        println!("✓ WordPress Playground stopped");
        return Ok(None);
    }
    ...
    if !has_ddev_config(project_path) {
        return Err(miette::miette!(
            "No DDEV or Playground project in {}. Stop is only for WordPress/PHP projects.",
            project_path.display()
        ));
    }
...
pub async fn dev(session: AppzSession, args: crate::args::DevArgs) -> AppResult {
    ...
    // Handle `appz dev stop` for DDEV/Playground projects
    if matches!(args.command, Some(DevSubcommand::Stop)) {
        return dev_stop(&project_path, args.playground).await;
    }

    let force_playground = args.playground;
    ...
    match detect_framework_record(options).await {
        Ok(Some((framework, _version, package_manager))) => {
            ...
            // Handle runtime setup for supported PHP/CMS frameworks
            // (WordPress, Drupal, Laravel, Jigsaw, Sculpin, Kirby, Statamic, etc.)
            //
            // For WordPress with --playground or when DDEV is unavailable, use Playground.
            // For other PHP frameworks, DDEV is still required.
            let resolved_runtime: Option<Arc<dyn blueprint::WordPressRuntime>> =
                if let Some(ref slug) = framework.slug {
                    if wp_runtime::is_wordpress_framework(slug) && (force_playground || !is_ddev_available()) {
                        // WordPress with Playground
                        Some(wp_runtime::resolve(&project_path, force_playground)?)
                    } else if is_ddev_supported_framework(slug) {
                        // DDEV path (WordPress without --playground, or other PHP frameworks)
                        if !is_ddev_available() {
                            return Err(miette::miette!(
                        ...
                        Some(Arc::new(blueprint::DdevRuntime::new()) as Arc<dyn blueprint::WordPressRuntime>)
                    } else {
                        None
                    }
                } else {
            ...
            if let Some(ref runtime) = resolved_runtime {
                if let Some(ref slug) = framework.slug {
                    if runtime.slug() == "ddev" {
                        // DDEV setup path (existing behavior)
                        if !has_ddev_config(&project_path) {
                        ...
                        if !runtime.check_connectivity(&project_path) {
                            eprintln!();
                        ...
                        // Playground setup path
                        println!("⚙️  Configuring WordPress Playground...");
                        runtime.configure(&project_path, "wordpress", None)
                            .map_err(|e| miette::miette!("{}", e))?;
            ...
            // Install step: skip for runtime-managed projects (DDEV or Playground)
            let skip_install_for_ddev = resolved_runtime.is_some()
                && framework.slug.as_ref().is_some_and(|s| {
                    is_ddev_supported_framework(s) && (has_ddev_config(&project_path)
            ...
            if skip_install_for_ddev {
                // WordPress runtime: wp core install as the install step
                if framework.slug.as_deref() == Some("wordpress") {
                    if let Some(ref runtime) = resolved_runtime {
                        println!("✓ Using WordPress install (wp core install via {})", runtime.name());
                        let is_installed = runtime.wp_is_installed(&project_path);
                    ...
                    println!("✓ Skipping install for {} project", resolved_runtime.as_ref().map(|r| r.name()).unwrap_or("DDEV"));
                }
            } else {
            ...
            // For runtime-managed frameworks: the runtime serves the site
            let use_runtime_dev = resolved_runtime.is_some()
                && framework.slug.as_ref().is_some_and(|s| {
                    is_ddev_supported_framework(s) && (has_ddev_config(&project_path)
                        || resolved_runtime.as_ref().map(|r| r.slug() == "playground").unwrap_or(false))
                });
            ...
            let result = if use_runtime_dev {
                if let Some(ref runtime) = resolved_runtime {
                    ...
                    let _ = runtime.open_browser(&project_path);
                    runtime.stream_logs(&project_path)
                        .map_err(|e| miette::miette!("{}", e))
                } else {
                    Ok(())
                }
