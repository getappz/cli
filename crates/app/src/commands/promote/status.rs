//! Promotion status polling logic
...
#[instrument(skip_all)]
pub async fn promote_status(session: AppzSession, timeout: Duration) -> AppResult {
...
pub async fn poll_promote_status(
    ...
    let promote_timeout = start_time + timeout;
    ...
    let mut spinner_message = if deployment.is_some() {
        "Promote in progress".to_string()
    } else {
        format!("Checking promotion status of {}", project_name)
    };
    ...
    loop {
        ...
        if !matches!(job_status, Some("pending") | Some("in-progress")) {
            ...
            return Err(miette::miette!("Unknown promotion status"));
        }
            ...
            || Instant::now() >= promote_timeout
        {
            ...
            return Err(miette::miette!("Promotion timeout"));
        }
...
async fn render_job_failed(
    ...
    Err(miette::miette!("Promotion failed"))
}
