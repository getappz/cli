...
macro_rules! d {
    ($path:expr) => { Detector { path: $path, is_glob: false, match_content: None, match_package: None } };
    ($path:expr, $re:expr) => { Detector { path: $path, is_glob: false, match_content: Some($re), match_package: None } };
}
macro_rules! d_pkg {
    ($pkg:expr) => { Detector { path: "package.json", is_glob: false, match_content: None, match_package: Some($pkg) } };
}
macro_rules! d_glob {
    ($path:expr) => { Detector { path: $path, is_glob: true, match_content: None, match_package: None } };
    ($path:expr, $re:expr) => { Detector { path: $path, is_glob: true, match_content: Some($re), match_package: None } };
}
macro_rules! cmds {
    ($b:expr, $i:expr, $d:expr) => { LifecycleCommands { build: $b, install: $i, dev: $d, test: None, lint: None, format: None } };
    ($b:expr, $i:expr, $d:expr, $t:expr, $l:expr, $f:expr) => { LifecycleCommands { build: $b, install: $i, dev: $d, test: $t, lint: $l, format: $f } };
}
macro_rules! fw {
    ...
    ($name:expr, $slug:expr, $plugin:expr, $ver:expr, $def:expr,
     ...
     $(,)?) => {
        Framework {
            ...
            version_files: $ver,
            default_version: $def,
            detectors: DetectionCriteria { every: $every, some: $some },
            commands: $commands,
...
macro_rules! C { ($s:expr) => { Some($s) } }
macro_rules! NONE { () => { None } }
...
pub static BLITZJS_LEGACY: Framework = fw_ext!(
    ...
    &["hydrogen", "vite", "node"], DetectionConfidence::Strong,
    None, Some("NEXT_PUBLIC_"),
);
pub static NEXTJS: Framework = fw_ext!(
    "Next.js", "nextjs", "node", &[".nvmrc", ".node-version"], "lts",
    &[d_pkg!("next")], &[],
    cmds!(C!("next build"), C!("npm install"), C!("next dev --port $PORT")),
    &[], DetectionConfidence::Strong,
    Some(".next"), Some("NEXT_PUBLIC_"),
);
pub static GATSBY: Framework = fw_ext!(
    "Gatsby.js", "gatsby", "node", &[], "lts",
    &[d_pkg!("gatsby")], &[],
    cmds!(C!("gatsby build"), C!("npm install"), C!("gatsby develop --port $PORT")),
    &[], DetectionConfidence::Strong,
    Some("public"), Some("GATSBY_"),
);
pub static REMIX: Framework = fw_ext!(
    "Remix", "remix", "node", &[], "lts",
    &[], &[d_pkg!("@remix-run/dev"), d!("remix.config.js"), d!("remix.config.mjs")],
    cmds!(C!("remix build"), C!("npm install"), C!("remix dev")),
    &["vite", "node"], DetectionConfidence::Strong,
    None, None,
);
pub static REACT_ROUTER: Framework = fw_ext!(
    "React Router", "react-router", "node", &[], "lts",
    &[], &[d!("react-router.config.js"), d!("react-router.config.ts")],
    cmds!(C!("react-router build"), C!("npm install"), C!("react-router dev")),
    &["vite", "node"], DetectionConfidence::Strong,
    Some("build"), None,
);
pub static ASTRO: Framework = fw_ext!(
    "Astro", "astro", "node", &[], "lts",
    &[d_pkg!("astro")], &[],
    cmds!(C!("astro build"), C!("npm install"), C!("astro dev --port $PORT")),
    &[], DetectionConfidence::Strong,
    Some("dist"), Some("PUBLIC_"),
);
pub static HEXO: Framework = fw!(
    "Hexo", "hexo", "node", &[], "lts",
    &[d_pkg!("hexo")], &[],
    cmds!(C!("hexo generate"), C!("npm install"), C!("hexo server --port $PORT")),
);
pub static ELEVENTY: Framework = fw!(
    "Eleventy", "eleventy", "node", &[], "lts",
    &[d_pkg!("@11ty/eleventy")], &[],
    cmds!(C!("npx @11ty/eleventy"), C!("npm install"), C!("npx @11ty/eleventy --serve --watch --port $PORT")),
);
pub static DOCUSAURUS_V2: Framework = fw!(
    "Docusaurus (v2+)", "docusaurus-2", "node", &[], "lts",
    &[], &[d_pkg!("@docusaurus/core")],
    cmds!(C!("docusaurus build"), C!("npm install"), C!("docusaurus start --port $PORT")),
);
pub static DOCUSAURUS_V1: Framework = fw!(
    "Docusaurus (v1)", "docusaurus", "node", &[], "lts",
    &[], &[d_pkg!("docusaurus")],
    cmds!(C!("docusaurus-build"), C!("npm install"), C!("docusaurus-start --port $PORT")),
);
pub static PREACT: Framework = fw_ext!(
    "Preact", "preact", "node", &[], "lts",
    &[d_pkg!("preact-cli")], &[],
    cmds!(C!("preact build"), C!("npm install"), C!("preact watch --port $PORT")),
    &[], DetectionConfidence::Strong,
    Some("build"), None,
);
pub static SOLIDSTART_V1: Framework = fw_ext!(
    "SolidStart (v1)", "solidstart-1", "node", &[], "lts",
    &[d_pkg!("solid-js"), d_pkg!("@solidjs/start")], &[],
    cmds!(C!("vinxi build"), C!("npm install"), C!("vinxi dev")),
    &["vite"], DetectionConfidence::Strong,
    Some(".output"), Some("VITE_"),
);
pub static SOLIDSTART_V0: Framework = fw_ext!(
    "SolidStart (v0)", "solidstart", "node", &[], "lts",
    &[d_pkg!("solid-js"), d_pkg!("solid-start")], &[],
    cmds!(C!("solid-start build"), C!("npm install"), C!("solid-start dev")),
    &["vite"], DetectionConfidence::Strong,
    Some(".output"), Some("VITE_"),
);
pub static DOJO: Framework = fw!(
    ...
    cmds!(C!("dojo build"), C!("npm install"), C!("dojo build -m dev -w -s -p $PORT")),
);
pub static EMBER: Framework = fw!(
    "Ember.js", "ember", "node", &[], "lts",
    &[], &[d_pkg!("ember-source"), d_pkg!("ember-cli")],
    cmds!(C!("ember build"), C!("npm install"), C!("ember serve --port $PORT")),
);
pub static VUE: Framework = fw_ext!(
    "Vue.js", "vue", "node", &[], "lts",
    &[d_pkg!("@vue/cli-service")], &[],
    cmds!(C!("vue-cli-service build"), C!("npm install"), C!("vue-cli-service serve --port $PORT")),
    &[], DetectionConfidence::Strong,
    Some("dist"), Some("VUE_APP_"),
);
pub static SCULLY: Framework = fw!(
    "Scully", "scully", "node", &[], "lts",
    &[d_pkg!("@scullyio/init")], &[],
    cmds!(C!("ng build && scully"), C!("npm install"), C!("ng serve --port $PORT")),
);
pub static IONIC_ANGULAR: Framework = fw!(
    "Ionic Angular", "ionic-angular", "node", &[], "lts",
    &[d_pkg!("@ionic/angular")], &[],
    cmds!(C!("ng build"), C!("npm install"), C!("ng serve --port $PORT")),
);
pub static ANGULAR: Framework = fw_ext!(
    "Angular", "angular", "node", &[], "lts",
    &[d_pkg!("@angular/cli")], &[],
    cmds!(C!("ng build"), C!("npm install"), C!("ng serve --port $PORT")),
    &[], DetectionConfidence::Strong,
    Some("dist"), None,
...
pub static POLYMER: Framework = fw!(
    "Polymer", "polymer", "node", &[], "lts",
    &[d_pkg!("polymer-cli")], &[],
    cmds!(C!("polymer build"), C!("npm install"), C!("polymer serve --port $PORT")),
);
pub static SVELTE: Framework = fw_ext!(
    "Svelte", "svelte", "node", &[], "lts",
    &[d_pkg!("svelte")], &[d_pkg!("sirv-cli")],
...
pub static SVELTEKIT_V0: Framework = fw_ext!(
    ...
    cmds!(C!("svelte-kit build"), C!("npm install"), C!("svelte-kit dev --port $PORT")),
    &["vite"], DetectionConfidence::Strong,
    Some("public"), Some("VITE_"),
);
pub static SVELTEKIT: Framework = fw_ext!(
    "SvelteKit", "sveltekit-1", "node", &[], "lts",
    &[d_pkg!("@sveltejs/kit")], &[],
    cmds!(C!("vite build"), C!("npm install"), C!("vite dev --port $PORT")),
    &["vite"], DetectionConfidence::Strong,
    None, None,
);
pub static IONIC_REACT: Framework = fw_ext!(
    "Ionic React", "ionic-react", "node", &[], "lts",
    &[d_pkg!("@ionic/react")], &[],
    cmds!(C!("react-scripts build"), C!("npm install"), C!("react-scripts start")),
    &[], DetectionConfidence::Strong,
    Some("build"), None,
);
pub static CRA: Framework = fw_ext!(
    "Create React App", "create-react-app", "node", &[], "lts",
    &[], &[d_pkg!("react-scripts"), d_pkg!("react-dev-utils")],
    cmds!(C!("react-scripts build"), C!("npm install"), C!("react-scripts start")),
    &[], DetectionConfidence::Strong,
    Some("build"), Some("REACT_APP_"),
);
pub static GRIDSOME: Framework = fw!(
    "Gridsome", "gridsome", "node", &[], "lts",
    &[d_pkg!("gridsome")], &[],
    cmds!(C!("gridsome build"), C!("npm install"), C!("gridsome develop -p $PORT")),
);
pub static UMIJS: Framework = fw!(
    "UmiJS", "umijs", "node", &[], "lts",
    &[d_pkg!("umi")], &[],
    cmds!(C!("umi build"), C!("npm install"), C!("umi dev --port $PORT")),
);
pub static SAPPER: Framework = fw!(
    "Sapper", "sapper", "node", &[], "lts",
    &[d_pkg!("sapper")], &[],
    cmds!(C!("sapper export"), C!("npm install"), C!("sapper dev --port $PORT")),
);
pub static SABER: Framework = fw!(
    "Saber", "saber", "node", &[], "lts",
    &[d_pkg!("saber")], &[],
    cmds!(C!("saber build"), C!("npm install"), C!("saber --port $PORT")),
);
pub static STENCIL: Framework = fw!(
    "Stencil", "stencil", "node", &[], "lts",
    &[d_pkg!("@stencil/core")], &[],
    cmds!(C!("stencil build"), C!("npm install"), C!("stencil build --dev --watch --serve --port $PORT")),
);
pub static NUXT: Framework = fw_ext!(
    "Nuxt", "nuxtjs", "node", &[], "lts",
    &[], &[d_pkg!("nuxt"), d_pkg!("nuxt3"), d_pkg!("nuxt-edge"), d_pkg!("nuxt-nightly")],
    cmds!(C!("nuxt build"), C!("npm install"), C!("nuxt dev")),
    &["nitro"], DetectionConfidence::Strong,
    Some("dist"), Some("NUXT_ENV_"),
);
pub static REDWOODJS: Framework = fw_ext!(
    "RedwoodJS", "redwoodjs", "node", &[], "lts",
    &[d_pkg!("@redwoodjs/core")], &[],
    cmds!(C!("yarn rw deploy vercel"), C!("yarn install"), C!("yarn rw dev")),
    &[], DetectionConfidence::Strong,
    None, Some("REDWOOD_ENV_"),
);
...
pub static JEKYLL: Framework = fw!(
    ...
    &[d!("_config.yml")], &[],
    cmds!(C!("jekyll build"), C!("bundle install"), C!("bundle exec jekyll serve --watch --port $PORT")),
);
pub static BRUNCH: Framework = fw!(
    "Brunch", "brunch", "node", &[], "lts",
    &[], &[d_pkg!("brunch"), d!("brunch-config.js")],
    cmds!(C!("brunch build --production"), C!("npm install"), C!("brunch watch --server --port $PORT")),
);
pub static MIDDLEMAN: Framework = fw!(
    "Middleman", "middleman", "ruby", &[], "latest",
    &[d!("config.rb")], &[],
    cmds!(C!("bundle exec middleman build"), C!("bundle install"), C!("bundle exec middleman server -p $PORT")),
);
...
pub static VITE: Framework = fw_ext!(
    ...
    &[d_pkg!("vite")], &[],
    cmds!(C!("vite build"), C!("npm install"), C!("vite dev --port $PORT")),
    &["ionic-react"], DetectionConfidence::Strong,
    Some("dist"), Some("VITE_"),
);
pub static TANSTACK_START: Framework = fw!(
    "TanStack Start", "tanstack-start", "node", &[], "lts",
    &[d_pkg!("@tanstack/start")], &[],
    cmds!(C!("npm run build"), C!("npm install"), C!("npm run dev")),
);
pub static VITEPRESS: Framework = fw!(
    "VitePress", "vitepress", "node", &[], "lts",
    &[d_pkg!("vitepress")], &[],
    cmds!(C!("vitepress build"), C!("npm install"), C!("vitepress dev --port $PORT")),
);
pub static VUEPRESS: Framework = fw!(
    "VuePress", "vuepress", "node", &[], "lts",
    &[d_pkg!("vuepress")], &[],
    cmds!(C!("vuepress build"), C!("npm install"), C!("vuepress dev --port $PORT")),
);
pub static PARCEL: Framework = fw!(
    "Parcel", "parcel", "node", &[], "lts",
    &[d_pkg!("parcel")], &[],
    cmds!(C!("parcel build"), C!("npm install"), C!("parcel serve --port $PORT")),
);
// ── Python frameworks (mise_plugin: "python") ───────────
pub static FASTAPI: Framework = fw!(
    "FastAPI", "fastapi", "python", &[], "latest",
    &[d_pkg!("fastapi")], &[],
    cmds!(NONE!(), C!("pip install -r requirements.txt"), C!("uvicorn main:app")),
);
pub static FLASK: Framework = fw!(
    "Flask", "flask", "python", &[], "latest",
    &[d_pkg!("flask")], &[],
    cmds!(NONE!(), C!("pip install -r requirements.txt"), C!("flask run")),
);
pub static FASTHTML: Framework = fw!(
    "FastHTML", "fasthtml", "python", &[], "latest",
    &[d_pkg!("fasthtml")], &[],
    cmds!(NONE!(), C!("pip install -r requirements.txt"), C!("python main.py")),
);
pub static DJANGO: Framework = fw!(
    "Django", "django", "python", &[], "latest",
    &[d_pkg!("django")], &[],
    cmds!(C!("python manage.py collectstatic"), C!("pip install -r requirements.txt"), C!("python manage.py runserver")),
);
// ── Other backend frameworks (mise_plugin: "node") ──────
pub static ASH: Framework = fw!(
    "Ash", "ash", "elixir", &[], "latest",
    &[d!("mix.exs", "ash")], &[],
    cmds!(C!("mix compile"), C!("mix deps.get"), C!("mix phx.server")),
);
pub static EVE: Framework = fw!(
    "eve", "eve", "python", &[], "latest",
    &[d_pkg!("eve")], &[],
    cmds!(NONE!(), C!("pip install -r requirements.txt"), NONE!())
);
pub static NITRO: Framework = fw_ext!(
    "Nitro", "nitro", "node", &[], "lts",
...
pub static SANITY: Framework = fw_ext!(
    "Sanity", "sanity", "node", &[], "lts",
    &[d_pkg!("sanity")], &[],
    cmds!(C!("npm run build"), C!("npm install"), C!("npm run dev")),
    &[], DetectionConfidence::Strong,
    None, Some("SANITY_STUDIO_"),
);
pub static SANITY_V2: Framework = fw_ext!(
    "Sanity (v2 - legacy)", "sanity-v2-legacy", "node", &[], "lts",
    &[d_pkg!("@sanity/core")], &[],
    cmds!(C!("sanity build"), C!("npm install"), C!("sanity start --port $PORT")),
    &[], DetectionConfidence::Strong,
    None, Some("SANITY_STUDIO_"),
);
pub static STORYBOOK: Framework = fw_ext!(
    "Storybook", "storybook", "node", &[], "lts",
    &[d_pkg!("@storybook/react")], &[],
    cmds!(C!("build-storybook"), C!("npm install"), C!("storybook dev --port $PORT")),
...
pub static PYTHON: Framework = fw!(
    "Python", "python", "python", &[".python-version"], "latest",
    &[], &[d!("requirements.txt"), d!("pyproject.toml"), d!("setup.py"), d!("Pipfile")],
    cmds!(NONE!(), NONE!(), NONE!())
);
pub static RUBY_RUNTIME: Framework = fw!(
    "Ruby", "ruby", "ruby", &[".ruby-version"], "latest",
    &[], &[d!("Gemfile"), d!("Gemfile.lock")],
    cmds!(NONE!(), NONE!(), NONE!())
);
pub static RUST_RUNTIME: Framework = fw!(
    "Rust", "rust", "rust", &["rust-toolchain.toml", "rust-toolchain"], "latest",
    &[], &[d!("Cargo.toml")],
    cmds!(NONE!(), NONE!(), NONE!())
);
pub static RUST_AXUM: Framework = fw_ext!(
    "Axum", "axum", "rust", &[], "latest",
    &[d_pkg!("axum")], &[],
    cmds!(C!("cargo build"), NONE!(), C!("cargo run")),
    &["rust"], DetectionConfidence::Strong,
    None, None,
);
pub static RUST_ACTIX: Framework = fw_ext!(
    "Actix Web", "actix-web", "rust", &[], "latest",
    &[d_pkg!("actix-web")], &[],
    cmds!(C!("cargo build"), NONE!(), C!("cargo run")),
    &["rust"], DetectionConfidence::Strong,
    None, None,
...
pub static TEMPL: Framework = fw_ext!(
    "Templ", "templ", "go", &[], "latest",
    &[d!("go.mod", "a-h/templ")], &[],
    cmds!(C!("go build -o ./bin/main ."), NONE!(), C!("templ generate && go run .")),
    &["go"], DetectionConfidence::Strong,
    None, None,
);
// ── Ruby frameworks (mise_plugin: "ruby") ───────────────
pub static RAILS: Framework = fw_ext!(
    "Ruby on Rails", "rails", "ruby", &[], "latest",
    &[d!("Gemfile", "rails")], &[],
    cmds!(C!("rails assets:precompile"), C!("bundle install"), C!("rails server -p $PORT")),
    &["ruby"], DetectionConfidence::Strong,
    Some("public"), None,
);
pub static SINATRA: Framework = fw_ext!(
    "Sinatra", "sinatra", "ruby", &[], "latest",
    &[d!("Gemfile", "sinatra")], &[],
...
pub static HANAMI: Framework = fw_ext!(
    "Hanami", "hanami", "ruby", &[], "latest",
    &[d!("Gemfile", "hanami")], &[],
    cmds!(C!("bundle exec hanami assets precompile"), C!("bundle install"), C!("bundle exec hanami server")),
    &["ruby"], DetectionConfidence::Strong,
    Some("public"), None,
);
// ── Java/JVM frameworks (mise_plugin: "java") ───────────
pub static JAVA_RUNTIME: Framework = fw!(
    "Java", "java", "java", &[".java-version"], "latest",
    &[], &[d!("pom.xml"), d!("build.gradle"), d!("build.gradle.kts"), d!("settings.gradle")],
    cmds!(NONE!(), NONE!(), NONE!())
);
pub static SPRING_BOOT: Framework = fw_ext!(
    "Spring Boot", "spring-boot", "java", &[], "latest",
    &[d!("pom.xml", "spring-boot"), d!("build.gradle", "spring-boot")], &[],
    cmds!(C!("mvn package"), C!("mvn install"), C!("mvn spring-boot:run")),
    &["java"], DetectionConfidence::Strong,
    Some("target"), None,
);
pub static QUARKUS: Framework = fw_ext!(
    "Quarkus", "quarkus", "java", &[], "latest",
...
pub static MICRONAUT: Framework = fw_ext!(
    "Micronaut", "micronaut", "java", &[], "latest",
...
pub static TURBOREPO: Framework = fw!(
    "Turborepo", "turbo", "turbo", &[], "latest",
    &[], &[d!("turbo.json"), d!("turbo.jsonc")],
    cmds!(C!("turbo run build"), C!("npm install"), C!("turbo run dev")),
);
...
pub static TERRAFORM: Framework = fw!(
    "Terraform", "terraform", "terraform", &[".terraform-version"], "latest",
    &[], &[d_glob!("*.tf")],
