//! GitHub release discovery + download helpers for `appz update`.
//!
//! Split out of the top-level `update` module so the network/release logic
...
pub(crate) fn binary_name() -> &'static str {
    if cfg!(windows) { "appz.exe" } else { "appz" }
}
...
mod tests {
    ...
    fn parse_checksum_finds_the_matching_asset() {
        let sums = "aaaa  appz-x86_64-apple-darwin.tar.gz\n\
                    bbbb  appz-x86_64-pc-windows-msvc.zip\n";
        assert_eq!(
            parse_checksum(sums, "appz-x86_64-pc-windows-msvc.zip").as_deref(),
            Some("bbbb")
        );
    ...
    fn parse_checksum_ignores_prefix_substring_collisions() {
        ...
        let sums = "cccc  other-appz-x86_64-apple-darwin.tar.gz\n";
        assert_eq!(
            parse_checksum(sums, "appz-x86_64-apple-darwin.tar.gz").as_deref(),
            None
        );
    }
