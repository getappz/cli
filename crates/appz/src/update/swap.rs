//! MCP-safe self-replacement of the `appz` binary.
//!
//! Replacing the running binary must not break a live `appz mcp` stdio
...
fn schedule_deferred_swap_windows(new_binary: &Path, target: &Path) -> Result<(), String> {
    ...
    let staged = tmp.join(format!("appz-new-{pid}.exe"));
    std::fs::copy(new_binary, &staged).map_err(|e| format!("stage new binary: {e}"))?;
    let bat = tmp.join(format!("appz-swap-{pid}.bat"));
    let script = format!(
        "@echo off\r\n\
         :wait\r\n\
...
/// PIDs of *other* running `appz` processes, excluding this process.
///
...
/// live `appz mcp` server keeps running its loaded image safely, and killing
/// it would break the very session performing the update.
pub(crate) fn find_killable_pids() -> Vec<u32> {
    let self_pid = std::process::id();
    let raw = list_appz_pids_raw();
...
mod tests {
    ...
    fn parse_other_pids_reads_tasklist_csv() {
        let raw = "\"appz.exe\",\"111\",\"Console\",\"1\",\"12,345 K\"\n\
                   \"appz.exe\",\"222\",\"Console\",\"1\",\"12,345 K\"\n";
    ...
    fn replace_binary_puts_new_bytes_in_place() {
        let dir = std::env::temp_dir().join(format!("appz-swap-test-{}-rb", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();

        let target = dir.join(if cfg!(windows) { "appz.exe" } else { "appz" });
