...
import { parseScrapeRequestBody, SCRAPE_DEFAULTS, } from "../scrape";
describe("parseScrapeRequestBody", () => {
    ...
    it("parses engine option", () => {
        ...
        if (nativeResult.ok)
            expect(nativeResult.data.engine).toBe("native");
        ...
        if (autoResult.ok)
            expect(autoResult.data.engine).toBe("auto");
        ...
        if (invalidResult.ok)
            expect(invalidResult.data.engine).toBeUndefined();
    });
});
