...
import { createAgentJob, getAgentJob, getAgentTeamId, updateAgentFailure, } from "../services/agent-store";
// ---------------------------------------------------------------------------
// POST /v2/agent — create agent job (async, Firecrawl-compatible)
// ---------------------------------------------------------------------------
export async function agentController(c) {
    let rawBody;
    try {
...
// ---------------------------------------------------------------------------
export async function agentStatusController(c) {
...
// ---------------------------------------------------------------------------
export async function agentCancelController(c) {
    const jobId = c.req.param("jobId");
    if (!jobId) {
        return c.json({ success: false, error: "Missing jobId" }, 400);
    }
