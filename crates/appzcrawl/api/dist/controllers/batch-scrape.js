...
// Adapted from firecrawl/apps/api/src/controllers/v2/batch-scrape.ts
// ---------------------------------------------------------------------------
export async function batchScrapeController(c) {
    let rawBody;
    try {
        rawBody = await c.req.json();
    }
    catch {
        return c.json({ success: false, error: "Invalid JSON body; expected { urls: [...] }" }, 400);
    }
    ...
    const wantsScreenshot = body.formats?.some((f) => f === "screenshot" || f === "screenshot@fullPage");
    ...
    if (c.env.SCRAPE_QUEUE) {
        ...
        logger.warn("[batch-scrape] SCRAPE_QUEUE not bound; will run synchronously in background", { batchId: id });
        // Process async in background
        const { processScrapeQueue } = await import("../services/scrape-consumer");
        c.executionCtx.waitUntil(processScrapeQueue({
