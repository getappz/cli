...
import { getWasmDiagnostics, getInitError as getWasmInitError, } from "../services/wasm-processor";
async function time(fn, iterations, warmupRuns) {
...
const BENCHMARKS = {
    ...
    extractBaseHref: (env, html) => ({
        ...
        container: () => container.extractBaseHref(env, html, "https://example.com"),
    }),
    ...
    transformHtml: (env, html) => ({
        wasm: () => wasmNative.transformHtml({
            html,
            url: "https://example.com",
            only_main_content: true,
        }),
        engine: () => engine.transformHtml(env, {
            html,
            url: "https://example.com",
            only_main_content: true,
        }),
        container: () => container.transformHtml(env, {
            html,
            url: "https://example.com",
            only_main_content: true,
        }),
    ...
    extractAttributes: (env, html) => ({
        wasm: () => wasmNative.extractAttributes(html, {
            selectors: [
                { selector: "a", attribute: "href" },
                { selector: "img", attribute: "src" },
            ],
        }),
        engine: () => engine.extractAttributes(env, html, {
            selectors: [
                { selector: "a", attribute: "href" },
                { selector: "img", attribute: "src" },
            ],
        }),
        container: () => container.extractAttributes(env, html, {
            selectors: [
                { selector: "a", attribute: "href" },
                { selector: "img", attribute: "src" },
            ],
    ...
    filterLinks: (env, _html) => ({
        ...
        container: () => container.filterLinks(env, {
            links: [
                "https://example.com/products/1",
                "https://example.com/products/2",
                "https://example.com/about",
    ...
    nativeSearch: (env, _html) => ({
        engine: () => engine.nativeSearch(env, { query: "cloudflare workers", num_results: 3 }),
        container: () => container.nativeSearch(env, {
...
// Controller
// ---------------------------------------------------------------------------
export async function benchmarkController(c) {
    ...
    return c.json({
        ...
        config: {
            ...
            wasmAvailable,
            engineAvailable,
            timerBehavior: "Deployed: timer advances only after I/O (Spectre mitigation). CPU-bound (WASM, Engine) show 0. Run wrangler dev locally for accurate CPU timings. See https://developers.cloudflare.com/workers/runtime-apis/performance/",
            ...(wasmAvailable
                ? {}
                : {
