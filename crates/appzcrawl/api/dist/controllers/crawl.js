...
import { cancelCrawlJob, createCrawlJob, getCrawlErrors, getCrawlJob, getCrawlResults, getOngoingCrawls, getRobotsBlocked, } from "../services/crawl-store";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
export async function crawlController(c) {
    let rawBody;
    try {
        rawBody = await c.req.json();
    }
    catch {
        return c.json({ success: false, error: "Invalid JSON body; expected { url: string }" }, 400);
    }
    ...
    try {
        ...
        return c.json({ success: false, error: e instanceof Error ? e.message : "Invalid URL" }, 400);
    }
    ...
    const wantsScreenshot = baseScrapeOpts.formats?.some((f) => f === "screenshot" || f === "screenshot@fullPage");
    const scrapeOptions = {
        ...baseScrapeOpts,
        ...(wantsScreenshot && {
    ...
    if (c.env.CRAWL_QUEUE) {
        ...
        logger.warn("[crawl] CRAWL_QUEUE not bound; will run synchronously in background", { crawlId: id });
        // Use waitUntil if available (Workers runtime) to not block response
        const { runCrawlAsync } = await import("../services/crawl-runner");
        c.executionCtx.waitUntil(runCrawlAsync(c.env, id).catch((e) => {
...
// ---------------------------------------------------------------------------
export async function crawlParamsPreviewController(c) {
    return c.json({
        success: true,
...
// ---------------------------------------------------------------------------
export async function crawlStatusController(c, _isBatchScrape = false) {
    ...
    const hasMore = crawl.status !== "completed" || crawl.completedCount > skip + data.length;
    const next = hasMore
...
// ---------------------------------------------------------------------------
export async function crawlCancelController(c) {
    const jobId = c.req.param("jobId");
    if (!jobId) {
        return c.json({ success: false, error: "Missing jobId" }, 400);
    }
    ...
    if (crawl.status === "cancelled") {
        return c.json({ success: true, status: "cancelled" }, 200);
    }
...
// ---------------------------------------------------------------------------
export async function ongoingCrawlsController(c) {
    const auth = c.get("auth");
    if (!auth) {
        return c.json({ success: false, error: "Unauthorized" }, 401);
    }
...
// ---------------------------------------------------------------------------
export async function crawlErrorsController(c) {
    const jobId = c.req.param("jobId");
    if (!jobId) {
        return c.json({ success: false, error: "Missing jobId" }, 400);
    }
