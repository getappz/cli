 ...
 */
import { createApiKey, listApiKeys, revokeApiKey, rotateApiKey, } from "../services/key-management";
// ---------------------------------------------------------------------------
// POST /v2/keys — Create a new API key
// ---------------------------------------------------------------------------
export async function createKeyController(c) {
...
// ---------------------------------------------------------------------------
export async function listKeysController(c) {
    const auth = c.get("auth");
    if (!auth)
        return c.json({ success: false, error: "Unauthorized" }, 401);
    try {
...
// ---------------------------------------------------------------------------
export async function revokeKeyController(c) {
    const auth = c.get("auth");
    if (!auth)
        return c.json({ success: false, error: "Unauthorized" }, 401);
    ...
    try {
        ...
        if (!revoked) {
            return c.json({ success: false, error: "Key not found or already revoked" }, 404);
        }
...
// ---------------------------------------------------------------------------
export async function rotateKeyController(c) {
    const auth = c.get("auth");
    if (!auth)
        return c.json({ success: false, error: "Unauthorized" }, 401);
    const keyId = Number.parseInt(c.req.param("keyId"), 10);
    ...
    try {
        ...
        return c.json({
            ...
            data: {
                ...
                key: result.fullKey,
                keyPrefix: result.keyPrefix,
                message: "Old key has been revoked. Save this new key securely. It will not be shown again.",
            },
        });
    }
    catch (e) {
