...
export async function mapController(c) {
    let rawBody;
    try {
    ...
    const timeoutId = timeoutMs > 0 ? setTimeout(() => abort.abort(), timeoutMs) : undefined;
    try {
        ...
        if (abort.signal.aborted) {
            return c.json({ success: false, error: "Map request timed out", code: "MAP_TIMEOUT" }, 408);
        }
