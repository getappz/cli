...
import { createScrapeJob, getScrapeJob, getScrapeTeamId, updateScrapeFailure, updateScrapeSuccess, } from "../services/scrape-store";
...
export async function scrapeController(c) {
    let rawBody;
    try {
    ...
    const wantsScreenshotFormat = body.formats?.some((f) => f === "screenshot" || f === "screenshot@fullPage");
    ...
    if (result.success) {
        ...
        if (!body.zeroDataRetention) {
            c.executionCtx.waitUntil(updateScrapeSuccess(c.env.DB, jobId, response.data).catch((e) => {
    ...
    if (!body.zeroDataRetention) {
        c.executionCtx.waitUntil(updateScrapeFailure(c.env.DB, jobId, result.error).catch((e) => {
...
// ---------------------------------------------------------------------------
export async function scrapeStatusController(c) {
    ...
    if (job.zeroDataRetention) {
        return c.json({
            success: false,
            error: "Zero data retention is enabled for this scrape. Status lookup is not available.",
        }, 400);
    }
