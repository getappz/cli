...
export const scrapeCache = sqliteTable("scrape_cache", {
    ...
    lookupIdx: index("idx_scrape_cache_lookup").on(table.urlHash, table.cacheKey, table.expiresAtMs),
}));
// ---------------------------------------------------------------------------
...
export const crawlJobs = sqliteTable("crawl_jobs", {
    ...
    teamStatusIdx: index("idx_crawl_jobs_team_status").on(table.teamId, table.status),
...
export const crawlRobotsBlocked = sqliteTable("crawl_robots_blocked", {
    /** FK to crawl_jobs.id. */
    crawlId: text("crawl_id").notNull(),
    ...
    uniqueUrl: index("idx_crawl_robots_blocked_unique").on(table.crawlId, table.url),
}));
// ---------------------------------------------------------------------------
...
export const crawlVisitedUrls = sqliteTable("crawl_visited_urls", {
    /** FK to crawl_jobs.id. */
    crawlId: text("crawl_id").notNull(),
