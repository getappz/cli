...
export function buildBrandingPrompt(input) {
    const { jsAnalysis, buttons, logoCandidates, brandName, pageTitle, pageUrl, backgroundCandidates, url, headerHtmlChunk, favicon, ogImage: _ogImage, heuristicLogoPick, } = input;
    ...
    if (jsAnalysis.fonts && jsAnalysis.fonts.length > 0) {
        ...
        for (const font of jsAnalysis.fonts) {
            ...
            const count = typeof font === "object" && font.count !== undefined ? font.count : "";
    ...
    const classifyHref = (href, hrefMatch) => {
        if (!href || !href.trim())
            return "none";
        if (hrefMatch)
            return "home";
    ...
    const getLogoCandidateMeta = (candidate) => {
        ...
        if (maxSide <= 24 || area <= 400)
            sizeLabel = "tiny";
        else if (maxSide <= 48 || area <= 1800)
            sizeLabel = "small";
        else if (maxSide <= 140 || area <= 12000)
            sizeLabel = "medium";
        else if (maxSide <= 320 || area <= 50000)
            sizeLabel = "large";
        else
            sizeLabel = "hero";
        const hrefType = classifyHref(candidate.href, candidate.indicators.hrefMatch);
        ...
        const hints = [];
        if (filenameHasLogo || srcHasLogo)
            hints.push("filename=logo");
        if (sizeLabel === "tiny" || sizeLabel === "small")
            hints.push("icon-sized");
        if (sizeLabel === "hero")
            hints.push("hero-sized");
        if (aspectRatio >= 3 && width >= 80)
            hints.push("wordmark-shaped");
        if (hrefType === "external")
            hints.push("external-link");
    ...
    if (allClasses.size > 0) {
        ...
        const frameworkHints = jsAnalysis.__framework_hints;
    ...
    if (buttons && buttons.length > 0) {
        ...
        for (let idx = 0; idx < buttons.length; idx++) {
            const bg = buttons[idx].background || "transparent";
        ...
        for (let idx = 0; idx < buttons.length; idx++) {
            ...
            if (btn.borderColor)
                prompt += `- Border Color: ${btn.borderColor}\n`;
            if (btn.borderRadius)
    ...
    if (logoCandidates && logoCandidates.length > 0) {
        ...
        if (favicon) {
            const faviconPreview = favicon.length > 100 ? `${favicon.substring(0, 100)}...` : favicon;
        ...
        for (let idx = 0; idx < logoCandidates.length; idx++) {
            ...
            const indicators = [];
            if (candidate.indicators.inHeader)
                indicators.push("header");
            if (candidate.indicators.altMatch)
                indicators.push("alt=logo");
            if (candidate.indicators.srcMatch)
                indicators.push("url=logo");
            if (candidate.indicators.classMatch)
                indicators.push("class=logo");
            if (candidate.indicators.hrefMatch)
                indicators.push("href=home");
            const meta = getLogoCandidateMeta(candidate);
            const urlPreview = candidate.src.length > 80
            ...
            const svgScoreLabel = candidate.logoSvgScore !== undefined
                ? `logoSvgScore:${Math.round(candidate.logoSvgScore)}`
                : "";
            const pos = candidate.position;
            const positionLabel = pos != null
