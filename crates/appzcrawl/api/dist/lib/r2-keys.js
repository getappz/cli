...
export function crawlPrefix(crawlId) {
 ...
 */
export async function deleteR2Objects(bucket, keys) {
    if (keys.length === 0)
        return;
 ...
 */
export async function storeOrInline(bucket, r2Key, json) {
 ...
 */
export async function getFromInlineOrR2(bucket, inlineJson, r2Key) {
    if (inlineJson)
        return inlineJson;
    if (!r2Key)
        return null;
