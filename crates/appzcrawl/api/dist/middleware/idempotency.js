...
export async function idempotencyMiddleware(c, next) {
    ...
    try {
        const existing = await c.env.DB.prepare("SELECT 1 FROM idempotency_keys WHERE idempotency_key = ? LIMIT 1")
            .bind(key)
            .first();
        if (existing) {
            return c.json({ success: false, error: "Idempotency key already used" }, 409);
        }
        ...
        await c.env.DB.prepare("INSERT INTO idempotency_keys (idempotency_key, created_at) VALUES (?, ?)")
            .bind(key, now)
            .run();
    }
    catch (e) {
