...
export async function getLinksFromSitemap({ sitemapUrl, urlsHandler, mode = "axios", maxAge = 0, zeroDataRetention, location, headers, }, logger, crawlId, sitemapsHit, abort, mock) {
    ...
    try {
        ...
        if (isGzip) {
            ...
            try {
                const shouldPrioritizeFireEngine = location && mode === "fire-engine" && useFireEngine;
                const forceEngine = [
                    ...(maxAge > 0 && useIndex ? ["index"] : []),
                ...
                const response = await scrapeURL("sitemap;" + crawlId, sitemapUrl, scrapeOptions.parse({
                    formats: ["rawHtml"],
                    useMock: mock,
                    maxAge,
                    ...(location ? { location } : {}),
        ...
        try {
            ...
            if (root && "sitemap" in root && root.sitemap) {
                ...
                const sitemapPromises = sitemapUrls.map(sitemapUrl => getLinksFromSitemap({
                    sitemapUrl,
                    urlsHandler,
                    mode,
                    zeroDataRetention,
                    ...
                    headers,
                    maxAge,
                }, logger, crawlId, sitemapsHit, abort, mock));
                ...
                if (xmlSitemaps.length > 0) {
                    const sitemapPromises = xmlSitemaps.map(sitemapUrl => getLinksFromSitemap({
        ...
        for (const instruction of instructions.instructions) {
            if (instruction.action === "recurse") {
                const sitemapPromises = instruction.urls.map(sitemapUrl => getLinksFromSitemap({
                    sitemapUrl,
                    urlsHandler,
                    mode,
                    zeroDataRetention,
