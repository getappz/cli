...
describe("Engine Forcing", () => {
    ...
    describe("getEngineForUrl", () => {
        ...
        it("should match wildcard patterns", () => {
            expect(getEngineForUrl("https://api.subdomain.com")).toBe("fire-engine;chrome-cdp");
            expect(getEngineForUrl("https://www.subdomain.com")).toBe("fire-engine;chrome-cdp");
        });
        ...
        it("should handle URLs with paths and query strings", () => {
            expect(getEngineForUrl("https://example.com/path?query=1")).toBe("playwright");
        });
        ...
        it("should throw error if not initialized", () => {
            delete config.FORCED_ENGINE_DOMAINS;
            expect(() => {
                const { getEngineForUrl: freshGetEngineForUrl, } = require("../engine-forcing");
                freshGetEngineForUrl("https://example.com");
            }).toThrow("Engine forcing not initialized");
        });
    });
});
//# sourceMappingURL=engine-forcing.test.js.map