...
export function isUrlBlocked(url, flags) {
    if (blob === null) {
    ...
    const decryptedUrl = blockedlist.find(decrypted => lowerCaseUrl === decrypted) || lowerCaseUrl;
    ...
    if (blob.allowedKeywords.some(keyword => lowerCaseUrl.includes(keyword.toLowerCase()))) {
        return false;
    }
    // Block exact matches
