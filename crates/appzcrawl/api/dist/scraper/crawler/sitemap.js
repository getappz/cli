...
async function _getSitemapXMLGZ(options) {
...
async function getSitemapXML(options) {
    ...
    const isLocationSpecified = options.location && options.location.country !== "us-generic";
    const forceEngine = [
        ...(options.maxAge > 0 && useIndex ? ["index"] : []),
    ...
    const response = await scrapeURL("sitemap;" + options.crawlId, options.url, scrapeOptions.parse({
        formats: ["rawHtml"],
    ...
    if (response.success &&
        ...
        throw new SitemapError(`Failed to fetch the sitemap from the website. The request failed with an error. This usually happens when: (1) The sitemap URL is incorrect, (2) The website is blocking access to the sitemap, (3) The website is down or unreachable, or (4) The sitemap requires authentication. Error details: ${response.error}`, response.error);
    }
    else {
        throw new SitemapError(`Failed to fetch the sitemap from the website. The server returned HTTP status code ${response.document.metadata.statusCode}. This usually means: (1) The sitemap doesn't exist at this URL (404), (2) Access is forbidden (403), or (3) The server encountered an error (5xx). Verify the sitemap URL is correct and accessible.`, response.document.metadata.statusCode);
    }
}
export async function scrapeSitemap(options) {
