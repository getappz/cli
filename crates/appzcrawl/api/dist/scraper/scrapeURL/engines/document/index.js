...
function getDocumentTypeFromContentType(contentType) {
    if (!contentType)
        return null;
...
export async function scrapeDocument(meta) {
    let response;
    let buffer;
    let proxyUsed = "basic";
    ...
    try {
        if (meta.documentPrefetch !== undefined && meta.documentPrefetch !== null) {
            ...
            const result = await fetchFileToBuffer(meta.rewrittenUrl ?? meta.url, meta.options.skipTlsVerification, {
        ...
        const documentType = getDocumentTypeFromContentType(response.headers.get("content-type")) ??
            getDocumentTypeFromUrl(response.url);
        const html = await converter.convertBufferToHtml(new Uint8Array(buffer), documentType);
        return {
            url: response.url,
...
export function documentMaxReasonableTime(meta) {
    return 15000;
}
