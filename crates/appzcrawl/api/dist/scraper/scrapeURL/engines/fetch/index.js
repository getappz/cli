...
import { getSecureDispatcher, InsecureConnectionError, } from "../utils/safeFetch";
import { saveMock } from "../../lib/mock";
import { TextDecoder } from "util";
export async function scrapeURLWithFetch(meta) {
    const mockOptions = {
    ...
    await specialtyScrapeCheck(meta.logger.child({ method: "scrapeURLWithFetch/specialtyScrapeCheck" }), Object.fromEntries(response.headers));
    return {
        url: response.url,
        html: response.body,
        statusCode: response.status,
        contentType: (response.headers.find(x => x[0].toLowerCase() === "content-type") ??
            [])[1] ?? undefined,
        proxyUsed: "basic",
    };
}
export function fetchMaxReasonableTime(meta) {
    return 15000;
}
