...
export const getTypography = () => {
    const pickFontStack = (el) => {
        return (getComputedStyleCached(el)
            .fontFamily?.split(",")
...
export const detectColorScheme = () => {
    ...
    const hasDarkIndicator = html.classList.contains("dark") ||
    ...
    const hasLightIndicator = html.classList.contains("light") ||
...
export const isValidBackgroundColor = (color) => {
    if (!color || typeof color !== "string")
        return false;
    const normalized = color.toLowerCase().trim();
    if (normalized === "transparent" || normalized === "rgba(0, 0, 0, 0)") {
...
export const getBackgroundCandidates = () => {
    const candidates = [];
    const colorFrequency = new Map();
    const allSampleElements = document.querySelectorAll("body, html, main, article, [role='main'], div, section");
    const sampleEls = Array.from(allSampleElements).slice(0, CONSTANTS.MAX_BACKGROUND_SAMPLES);
...
//# sourceMappingURL=brand-utils.js.map