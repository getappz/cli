...
import { getClassNameString, getComputedStyleCached, recordError, } from "./helpers";
...
export const findImages = () => {
    ...
    push(document.querySelector('link[rel*="icon" i]')
        ?.href, "favicon");
    ...
    const isHomeHref = (href) => {
        ...
        if (normalizedHref.startsWith("http://") ||
            ...
            try {
                ...
                if (!isSameBrandHost(currentHostname, linkHostname))
                    return false;
        ...
        if (segments.length === 1 && !normalizedHref.includes("."))
            return true;
        return false;
    };
    const collectLogoCandidate = (el, source) => {
        ...
        const imgSrc = el.src || "";
        if (imgSrc) {
            const ogImageSrc = document
                .querySelector('meta[property="og:image"]')
                ?.getAttribute("content") || "";
            const twitterImageSrc = document
                .querySelector('meta[name="twitter:image"]')
                ?.getAttribute("content") || "";
            const matchesOgOrTwitter = (ogImageSrc && imgSrc.includes(ogImageSrc)) ||
            ...
            if (matchesOgOrTwitter) {
                ...
                const headerNavSelector = 'header, nav, [role="banner"], #navbar, [id*="navbar" i], #taskbar, [id*="taskbar" i], [role="menubar"]';
                const inHeaderOrNav = el.closest(headerNavSelector) !== null;
                const hasStrongLogoContext = (inHeaderOrNav && hasHomeHref) ||
                    (inHeaderOrNav && /logo/i.test(imgSrc)) ||
                    (inHeaderOrNav &&
                        el.closest('[class*="logo" i], [id*="logo" i]') !== null) ||
                    /logo/i.test((el.getAttribute("class") || "") + (el.getAttribute("alt") || ""));
        ...
        const parentInHeader = parentLink && parentLink.closest(headerNavSelector) !== null;
        ...
        if (!inHeader && !parentInHeader) {
            ...
            if (taskbarOrMenubar) {
                ...
                if (barRect.top <= CONSTANTS.TASKBAR_TOP_THRESHOLD &&
                    barRect.width > 0 &&
                    barRect.height > 0) {
                    inTopLevelNav = true;
                }
            ...
            if (!inTopLevelNav && topLevelContainer) {
                ...
                const hasNavLikeContent = topLevelContainer.querySelector('nav, a[href="/"], a[href="./"]') !==
                    null;
        ...
        const isSmallFlagImage = rect.width <= 20 &&
            rect.height <= 20 &&
            el.src &&
            /flag|lang|country/i.test(el.src.toLowerCase());
        ...
        if (insideButton) {
            const isLogoInNavContext = finalInHeader && (hasHomeHref || hasLogoAriaLabel || hasLogoDataAttr);
            ...
            const elementHasLogoIndicators = /logo|brand/i.test(el.getAttribute("alt") || "") ||
                /logo|brand/i.test(el.getAttribute("aria-label") || "");
            if (!isLogoInNavContext &&
                !isLogoInTopBar &&
        ...
        const isInSearchForm = parent &&
            ((parent.tagName === "FORM" &&
                ...
                (parent.matches &&
                    parent.matches('form[class*="search"], form[id*="search"], button[class*="search"], button[id*="search"], [role="search"]')));
        const inSearchButton = el.closest('button[class*="search"], button[id*="search"], a[class*="search"], a[id*="search"]');
        const isSearchIcon = hasSearchClass ||
            hasSearchId ||
            hasSearchAriaLabel ||
            isInSearchForm ||
            !!inSearchButton;
        ...
        const isUIIcon = /icon|menu|hamburger|bars|close|times|cart|user|account|profile|settings|notification|bell|chevron|arrow|caret|dropdown/i.test(elementClasses) ||
        ...
        if (isUIIcon) {
            ...
            const hasExplicitLogoIndicator = /logo|brand|site-name|site-title/i.test(elementClasses) ||
        ...
        let logoSvgScore = 0;
        if (isSvg) {
            const svgWidth = rect.width || parseFloat(el.getAttribute("width") || "0") || 0;
            const svgHeight = rect.height || parseFloat(el.getAttribute("height") || "0") || 0;
        ...
        if (isSvg) {
            ...
            const imageHref = imageEl?.getAttribute("href") ||
            ...
            if (imageHref) {
                ...
                if (!srcMatch)
                    srcMatch = /logo/i.test(imageHref);
            }
            if (!src) {
                try {
                    ...
                    try {
                        ...
                        const parentAria = (parentLinkForSvg?.getAttribute("aria-label") || "").toLowerCase();
                        const inHeaderNavForSvg = el.closest('header, nav, [role="banner"], #navbar, [id*="navbar" i], #taskbar, [id*="taskbar" i], [role="menubar"]') !== null;
                        const strongLogoCandidate = inHeaderNavForSvg &&
                            parentLinkForSvg &&
        ...
        matches.forEach(el => {
            ...
            if (bgImageUrl) {
                ...
                const isVisible = rect.width > 0 &&
                    ...
                    style.visibility !== "hidden" &&
                    style.opacity !== "0";
                const hasReasonableSize = rect.width >= CONSTANTS.MIN_LOGO_SIZE &&
                    rect.height >= CONSTANTS.MIN_LOGO_SIZE;
                const inLogoContext = el.closest('[class*="logo" i], [id*="logo" i], header, nav, [role="banner"]') !== null;
                const parentLink = el.closest("a");
                const hasLogoDataAttr = el.getAttribute("data-framer-name")?.toLowerCase().includes("logo") ||
                    el.getAttribute("data-name")?.toLowerCase().includes("logo");
                const hasLogoAriaLabel = parentLink &&
                    /logo|home|brand/i.test(parentLink.getAttribute("aria-label") || "");
                const hasHomeHrefCheck = parentLink && isHomeHref(parentLink.getAttribute("href") || "");
                const hasStrongLogoIndicators = elHasLogoClass ||
                    hasLogoDataAttr ||
                    hasLogoAriaLabel ||
                    hasHomeHrefCheck;
    ...
    const allDivsWithBgImage = allElementsWithBg.filter(el => {
        ...
        const hasLogoAriaLabel = parentLink &&
            /logo|home|brand/i.test(parentLink.getAttribute("aria-label") || "");
        const hasHomeHrefCheck = parentLink && isHomeHref(parentLink.getAttribute("href") || "");
        const inHeaderNavCheck = el.closest('header, nav, [role="banner"]') !== null;
        return (hasLogoDataAttr ||
            hasLogoClass ||
    ...
    allSvgs.forEach(svg => {
        ...
        const isSearchIcon = hasSearchId ||
            hasSearchClass ||
            hasSearchAriaLabel ||
            hasSearchTitle ||
            isInSearchForm ||
        ...
        const isUIIcon = /icon|menu|hamburger|bars|close|times|cart|user|account|profile|settings|notification|bell|chevron|arrow|caret|dropdown/i.test(svgClass) ||
        ...
        const inHeaderNav = svg.closest('header, nav, [role="banner"], #navbar, [id*="navbar" i], #taskbar, [id*="taskbar" i], [role="menubar"]');
        ...
        if (isUIIcon) {
            const hasExplicitLogoIndicator = hasLogoId ||
                hasLogoClass ||
                hasLogoAriaLabel ||
                hasLogoTitle ||
                inLogoContainer;
    ...
    const isHomeLinkSource = (c) => typeof c.source === "string" &&
    ...
    if (candidatesToPick.length > 0) {
        const best = candidatesToPick.reduce((best, candidate) => {
            ...
            if (!candidate.indicators.classMatch && best.indicators.classMatch)
                return best;
            const candidateTooSmall = candidate.position.width < CONSTANTS.MIN_LOGO_SIZE ||
                candidate.position.height < CONSTANTS.MIN_LOGO_SIZE;
            const bestTooSmall = best.position.width < CONSTANTS.MIN_LOGO_SIZE ||
                best.position.height < CONSTANTS.MIN_LOGO_SIZE;
            if (candidateTooSmall && !bestTooSmall)
                return best;
            if (!candidateTooSmall && bestTooSmall)
                return candidate;
