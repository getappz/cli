...
export const resolveSvgUseElements = (svgClone, originalSvg) => {
    ...
    for (const useEl of useElements) {
        ...
        let referencedEl = originalSvg.querySelector("#" + CSS.escape(targetId));
        if (!referencedEl) {
        ...
        if (referencedEl && useEl.parentNode) {
            ...
            if (clonedRef.tagName === "symbol" || clonedRef.tagName === "SYMBOL") {
                ...
                if (useIdx != null)
                    wrapper.setAttribute(FC_IDX, useIdx);
                ...
                if (useIdx != null)
                    clonedContent.setAttribute(FC_IDX, useIdx);
...
export const resolveSvgStyles = (svg) => {
    ...
    const applyResolvedStyle = (clonedEl, originalEl, computed, prop) => {
        ...
        if (attrValue && attrValue.includes("var(")) {
            ...
            const isExplicit = originalEl.hasAttribute(prop) ||
                originalEl.style[prop];
            const isDifferent = svgDefaults[prop] !== undefined && value !== svgDefaults[prop];
    ...
    clonedWithIdx.forEach(clonedEl => {
        ...
        if (i < 0 || i >= originalElements.length)
            return;
