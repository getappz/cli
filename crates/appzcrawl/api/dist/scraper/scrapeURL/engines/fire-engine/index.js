...
import { fireEngineCheckStatus, StillProcessingError, } from "./checkStatus";
import { ActionError, EngineError, DNSResolutionError, SiteError, SSLError, UnsupportedFileError, FEPageLoadFailed, ProxySelectionError, } from "../../error";
...
async function performFireEngineScrape(meta, logger, request, mock, abort, production = true) {
    return withSpan("engine.fire-engine.perform_scrape", async (span) => {
        ...
        if (scrape.processing) {
            ...
            while (status === undefined) {
                ...
                try {
                    pollCount++;
                    status = await fireEngineCheckStatus(meta, logger.child({ method: "fireEngineCheckStatus" }), scrape.jobId, mock, abort, production);
                }
                catch (error) {
        ...
        await specialtyScrapeCheck(logger.child({
        ...
        setSpanAttributes(span, {
            ...
            "fire-engine.has_pdf": !!status.pdf,
            "fire-engine.job_id": scrape.jobId,
        });
        return status;
    });
}
export async function scrapeURLWithFireEngineChromeCDP(meta) {
    return withSpan("engine.fire-engine.chrome-cdp", async (span) => {
        ...
        const effectiveWait = meta.options.waitFor != null && meta.options.waitFor !== 0
        ...
        const actions = [
            ...
            ...(hasFormatOfType(meta.options.formats, "screenshot")
                ? [
                    {
                        type: "screenshot",
                        fullPage: hasFormatOfType(meta.options.formats, "screenshot")?.fullPage ||
                            false,
        ...
        const shouldAllowMedia = hasFormatOfType(meta.options.formats, "branding") ||
            youtubePostprocessor.shouldRun(meta, new URL(meta.rewrittenUrl ?? meta.url));
        const request = {
            ...
            saveScrapeResultToGCS: !meta.internalOptions.zeroDataRetention &&
        ...
        let response = await performFireEngineScrape(meta, meta.logger.child({
            ...
            request,
        }), request, meta.mock, meta.abort.asSignal(), true);
        ...
        const javascriptReturns = (response.actionResults ?? [])
            ...
            .map(x => {
            const rawReturn = x.result.return;
            try {
...
export async function scrapeURLWithFireEnginePlaywright(meta) {
    return withSpan("engine.fire-engine.playwright", async (span) => {
        ...
        let response = await performFireEngineScrape(meta, meta.logger.child({
            ...
            request,
        }), request, meta.mock, meta.abort.asSignal());
        if (!response.url) {
            meta.logger.warn("Fire-engine did not return the response's URL", {
                response,
                sourceURL: meta.url,
...
export async function scrapeURLWithFireEngineTLSClient(meta) {
...
export function fireEngineMaxReasonableTime(meta, engine) {
    const hasBranding = hasFormatOfType(meta.options.formats, "branding");
    const defaultWait = hasBranding ? BRANDING_DEFAULT_WAIT_MS : 0;
    const effectiveWait = meta.options.waitFor != null && meta.options.waitFor !== 0
        ? meta.options.waitFor
    ...
    if (engine === "tlsclient") {
        ...
        return (effectiveWait +
            (meta.options.actions?.reduce((a, x) => (x.type === "wait" ? (x.milliseconds ?? 2500) + a : 250 + a), 0) ?? 0) +
            30000);
    }
}
