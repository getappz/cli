...
import { ActionError, DNSResolutionError, EngineError, FEPageLoadFailed, ProxySelectionError, SSLError, SiteError, UnsupportedFileError, } from "../../error";
...
export const fireEngineURL = config.FIRE_ENGINE_BETA_URL ?? "<mock-fire-engine-url>";
export const fireEngineStagingURL = config.FIRE_ENGINE_STAGING_URL ?? "<mock-fire-engine-url>";
export async function fireEngineScrape(meta, logger, request, mock, abort, production = true) {
    ...
    if (successParse.success) {
        ...
        if (successParse.data.pageStatusCode === 415 &&
