...
export async function sendDocumentToIndex(meta, document) {
        ...
        try {
            ...
            try {
                await saveIndexToGCS(indexId, {
                    ...
                    pdfMetadata: document.metadata.numPages !== undefined
                        ? {
            ...
            try {
                await addIndexInsertJob({
                    ...
                    resolved_url: document.metadata.url ??
                        document.metadata.sourceURL ??
                        meta.rewrittenUrl ??
                        meta.url,
                    has_screenshot: document.screenshot !== undefined &&
                        meta.featureFlags.has("screenshot"),
                    has_screenshot_fullscreen: document.screenshot !== undefined &&
...
export async function scrapeURLWithIndex(meta) {
        ...
        if (domainSplitsHash.length === 0 ||
            ...
            try {
                maxAge = await Promise.race([
                    (async () => {
                        const { data, error } = await index_supabase_service.rpc("query_max_age", {
    ...
    const { data, error } = await index_supabase_service.rpc("index_get_recent_3", {
        p_url_hash: urlHash,
        p_max_age_ms: maxAge,
        p_is_mobile: meta.options.mobile,
        p_block_ads: meta.options.blockAds,
        p_feature_screenshot: meta.featureFlags.has("screenshot"),
        p_feature_screenshot_fullscreen: meta.featureFlags.has("screenshot@fullScreen"),
        p_location_country: meta.options.location?.country ?? null,
        p_location_languages: (meta.options.location?.languages?.length ?? 0) > 0
...
export function indexMaxReasonableTime(meta) {
    return 1500;
}
