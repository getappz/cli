...
import { PDFAntibotError, PDFInsufficientTimeError, PDFPrefetchFailed, RemoveFeatureError, EngineUnsuccessfulError, } from "../../error";
...
async function scrapePDFWithRunPodMU(meta, tempFilePath, base64Content, maxPages) {
    ...
    if (config.PDF_MU_V2_EXPERIMENT === "true" &&
...
export async function scrapePDF(meta) {
    ...
    if (!shouldParse) {
        if (meta.pdfPrefetch !== undefined && meta.pdfPrefetch !== null) {
            ...
            const file = await fetchFileToBuffer(meta.rewrittenUrl ?? meta.url, meta.options.skipTlsVerification, {
    ...
    const { response, tempFilePath } = meta.pdfPrefetch !== undefined && meta.pdfPrefetch !== null
        ...
        : await downloadFile(meta.id, meta.rewrittenUrl ?? meta.url, meta.options.skipTlsVerification, {
            headers: meta.options.headers,
            signal: meta.abort.asSignal(),
        });
    try {
        ...
        if (effectivePageCount * MILLISECONDS_PER_PAGE >
            (meta.abort.scrapeTimeout() ?? Infinity)) {
            throw new PDFInsufficientTimeError(effectivePageCount, effectivePageCount * MILLISECONDS_PER_PAGE + 5000);
        }
        let result = null;
        ...
        if (base64Content.length < MAX_FILE_SIZE &&
            ...
            try {
                ...
                }, tempFilePath, base64Content, maxPages);
...
export function pdfMaxReasonableTime(meta) {
