...
export function isIPPrivate(address) {
    if (!IPAddr.isValid(address))
        return false;
    const addr = IPAddr.parse(address);
    return addr.range() !== "unicast";
}
function createBaseAgent(skipTlsVerification) {
...
function attachSecurityCheck(agent) {
...
function makeSecureDispatcher(skipTlsVerification) {
...
function makeSecureDispatcherNoCookies(skipTlsVerification) {
...
const secureDispatcherNoCookiesSkipTlsVerification = makeSecureDispatcherNoCookies(true);
export const getSecureDispatcher = (skipTlsVerification = false) => skipTlsVerification ? secureDispatcherSkipTlsVerification : secureDispatcher;
// Use this for webhook delivery to avoid sending empty cookie headers
export const getSecureDispatcherNoCookies = (skipTlsVerification = false) => skipTlsVerification
