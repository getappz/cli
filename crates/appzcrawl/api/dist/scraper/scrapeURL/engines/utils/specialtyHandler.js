...
async function feResToFilePrefetch(logger, feRes, fileExtension, fileType, contentType) {
    if (!feRes?.file) {
        logger.warn(`No file in ${fileType} prefetch`);
        return null;
    }
    const filePath = path.join(os.tmpdir(), `tempFile-${crypto.randomUUID()}.${fileExtension}`);
...
async function feResToDocumentPrefetch(logger, feRes, contentType) {
...
export async function specialtyScrapeCheck(logger, headers, feRes) {
    ...
    if (isDocument) {
        throw new AddFeatureError(["document"], undefined, await feResToDocumentPrefetch(logger, feRes, contentType));
    }
    ...
    if (isOctetStream) {
        const isZipSignature = feRes?.file?.content.startsWith("UEsD") ||
            feRes?.content.startsWith("PK");
        const isOleSignature = feRes?.file?.content.startsWith("0M8R4K") ||
        ...
        if (isOleSignature) {
            ...
            const effectiveContentType = isDocUrl
                ? "application/msword"
                : contentType;
            throw new AddFeatureError(["document"], undefined, await feResToDocumentPrefetch(logger, feRes, effectiveContentType));
        }
    }
    // Check for PDF
    if (isPdf) {
