...
export class AddFeatureError extends Error {
    ...
    pdfPrefetch;
    documentPrefetch;
    constructor(featureFlags, pdfPrefetch, documentPrefetch) {
...
export class SSLError extends TransportableError {
    ...
    constructor(skipTlsVerification) {
        super("SCRAPE_SSL_ERROR", "An SSL/TLS certificate error occurred while trying to establish a secure connection to this website. " +
            (skipTlsVerification
...
export class SiteError extends TransportableError {
    ...
    constructor(errorCode) {
        const errorExplanations = {
            ERR_TUNNEL_CONNECTION_FAILED: "Firecrawl encountered an internal proxy error while establishing the tunnel.",
            ERR_TIMED_OUT: "The connection timed out. The server is not responding or is too slow.",
            ERR_BLOCKED_BY_CLIENT: "The request was blocked by the client, possibly due to an ad blocker or network policy.",
            ERR_CONNECTION_CLOSED: "The connection was closed unexpectedly by the server.",
            ERR_HTTP2_PROTOCOL_ERROR: "An HTTP/2 protocol error occurred. The server may have misconfigured HTTP/2.",
            ERR_EMPTY_RESPONSE: "The server closed the connection without sending any response.",
            ERR_PROXY_CONNECTION_FAILED: "Firecrawl encountered an internal proxy error while connecting to the proxy.",
            ERR_CONNECTION_RESET: "The connection was reset by the peer. The server may have dropped the connection.",
            ERR_TOO_MANY_REDIRECTS: "The page has too many redirects. The website may be misconfigured.",
        };
        const isProxyError = errorCode === "ERR_TUNNEL_CONNECTION_FAILED" ||
        ...
        super("SCRAPE_SITE_ERROR", `The URL failed to load in the browser with error code "${errorCode}". ${explanation} ${followUp}`);
...
export class ProxySelectionError extends TransportableError {
    constructor() {
        super("SCRAPE_PROXY_SELECTION_ERROR", "The specified proxy location could not be selected for this scrape request. This happens when the requested geographic location or proxy type is not available or is incompatible with other options in your request. To fix this: (1) Try a different location value (e.g., 'US', 'GB', 'DE'), (2) Remove the location parameter to use the default, or (3) Check that your proxy settings are compatible with other scrape options you've specified.");
    }
    serialize() {
        return {
            ...super.serialize(),
...
export class DNSResolutionError extends TransportableError {
    ...
    constructor(hostname) {
        super("SCRAPE_DNS_RESOLUTION_ERROR", `DNS resolution failed for hostname "${hostname}". This means the domain name could not be translated to an IP address. Possible causes: (1) The domain name is misspelled (check for typos), (2) The domain does not exist or has expired, (3) The DNS servers are temporarily unavailable, or (4) The domain was recently registered and DNS has not propagated yet. Please verify the URL is correct and the website exists.`);
...
export class NoCachedDataError extends TransportableError {
    constructor() {
        super("SCRAPE_NO_CACHED_DATA", "No cached data is available for this URL that meets your specified age requirements. This error occurs when you use the minAge parameter to request only cached data, but Firecrawl has no cached version of this URL (or no version within the specified age range). To resolve this, either remove the minAge parameter to allow a fresh scrape, or try again later after the URL has been scraped and cached.");
    }
    serialize() {
        return super.serialize();
    }
...
export class DocumentAntibotError extends TransportableError {
    constructor() {
        super("SCRAPE_DOCUMENT_ANTIBOT_ERROR", "Document scrape was prevented by anti-bot");
    }
    serialize() {
        return super.serialize();
    }
...
export class ScrapeJobCancelledError extends TransportableError {
    constructor() {
        super("SCRAPE_JOB_CANCELLED", "Scrape job was cancelled before completion.");
    }
    serialize() {
        return super.serialize();
    }
...
export class ScrapeRetryLimitError extends TransportableError {
    ...
    constructor(reason, stats) {
        super("SCRAPE_RETRY_LIMIT", `Scrape aborted after exceeding retry limit (${reason}).`);
        this.reason = reason;
        this.stats = stats;
    }
    serialize() {
