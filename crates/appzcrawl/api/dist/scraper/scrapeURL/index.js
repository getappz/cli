...
import { getPDFMaxPages, scrapeOptions, } from "../../controllers/v2/types";
import { logger as _logger } from "../../lib/logger";
import { buildFallbackList, getEngineMaxReasonableTime, scrapeURLWithEngine, shouldUseIndex, } from "./engines";
import { parseMarkdown } from "../../lib/html-to-markdown";
import { hasFormatOfType } from "../../lib/format-utils";
import { ActionError, AddFeatureError, EngineError, NoEnginesLeftError, PDFAntibotError, DocumentAntibotError, RemoveFeatureError, SiteError, UnsupportedFileError, SSLError, PDFInsufficientTimeError, IndexMissError, NoCachedDataError, DNSResolutionError, ZDRViolationError, PDFPrefetchFailed, DocumentPrefetchFailed, FEPageLoadFailed, EngineSnipedError, WaterfallNextEngineSignal, EngineUnsuccessfulError, ProxySelectionError, ScrapeRetryLimitError, } from "./error";
...
import { AbortManager, AbortManagerThrownError, } from "./lib/abortManager";
import { ScrapeJobTimeoutError, CrawlDenialError, ActionsNotSupportedError, } from "../../lib/error";
...
function buildFeatureFlags(url, options, internalOptions) {
    ...
    const isDocument = lowerPath.endsWith(".docx") ||
...
async function buildMetaObject(id, url, options, internalOptions, costTracking) {
    ...
    const abortHandle = options.timeout !== undefined
        ? setTimeout(() => abortController.abort(new ScrapeJobTimeoutError()), options.timeout)
        : undefined;
    return {
        id,
        url,
        ...
        logger,
        abortHandle,
        abort: new AbortManager(internalOptions.externalAbort, options.timeout !== undefined
            ? {
...
async function scrapeURLLoopIter(meta, engine, snipeAbort) {
    ...
    try {
        ...
        const needsMarkdown = hasMarkdown || hasChangeTracking || hasJson || hasSummary;
        let checkMarkdown;
        ...
        if (meta.internalOptions.teamId === "sitemap" ||
            ...
            meta.logger.debug("Skipping markdown conversion for quality check due to large HTML size", {
                htmlSize,
            ...
            checkMarkdown = await parseMarkdown(await htmlTransform(engineResult.html, meta.url, scrapeOptions.parse({ onlyMainContent: true })), { logger: meta.logger, requestId });
            if (checkMarkdown.trim().length === 0) {
                checkMarkdown = await parseMarkdown(await htmlTransform(engineResult.html, meta.url, scrapeOptions.parse({ onlyMainContent: false })), { logger: meta.logger, requestId });
            }
        }
        // Success factors
    ...
    return withSpan("scrape.engine_loop", async (span) => {
        ...
        if (meta.featureFlags.has("actions")) {
            if (fallbackList.length === 0 ||
                ...
                throw new ActionsNotSupportedError("Actions are not supported by any available engines. Actions require Fire Engine (fire-engine) to be enabled.");
            }
        }
        setSpanAttributes(span, {
        ...
        while (remainingEngines.length > 0) {
            ...
            if (!isFinite(waitUntilWaterfall) ||
                ...
                try {
                        ...
                        new Promise((_, reject) => {
                            timeouts.push(setTimeout(() => {
                                try {
                                    ...
                                    const usingDefaultTimeout = meta.abort.scrapeTimeout() === undefined;
                                    throw new ScrapeJobTimeoutError(usingDefaultTimeout
                    ...
                    if (error instanceof WrappedEngineError) {
                        if (error.error instanceof EngineError) {
                            ...
                            meta.logger.warn("Engine " +
                                error.engine +
                                " could not find the page in the index.", {
                                error: error.error,
                            });
                        }
                        else if (error.error instanceof AddFeatureError ||
                            ...
                            meta.logger.warn("An unexpected error happened while scraping with " +
                                error.engine +
                                ".", { error });
                        }
        ...
        setSpanAttributes(span, {
            ...
            "engine.engines_attempted": enginesAttempted.join(","),
            "engine.unsupported_features": result.unsupportedFeatures.size > 0
                ? Array.from(result.unsupportedFeatures).join(",")
                : undefined,
        });
        meta.winnerEngine = result.engine;
        let engineResult = result.result;
        for (const postprocessor of postprocessors) {
            if (postprocessor.shouldRun(meta, new URL(engineResult.url), engineResult.postprocessorsUsed)) {
...
export async function scrapeURL(id, url, options, internalOptions, costTracking) {
                ...
                if (!isRobotsTxtPath) {
                    try {
                        ...
                        if (!robotsTxt) {
                            ...
                            }, id, meta.logger, meta.abort.asSignal());
                            robotsTxt = content;
                        }
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    else if (error instanceof DocumentAntibotError &&
                        meta.internalOptions.forceEngine === undefined) {
                        if (meta.documentPrefetch !== undefined) {
                            meta.logger.error("Document was prefetched and still blocked by antibot, failing");
                            throw error;
                        }
                        else {
                            retryTracker.record("document_antibot", error);
                            meta.logger.debug("Document was blocked by anti-bot, prefetching with chrome-cdp");
                            meta.featureFlags = new Set([...meta.featureFlags].filter(x => x !== "document"));
                        }
                    }
                    else {
                        throw error;
            ...
            if (useIndex) {
                meta.logger.debug("scrapeURL index metrics", {
                    ...
                    timeTaken: Date.now() - startTime,
                    changeTrackingEnabled: !!hasFormatOfType(meta.options.formats, "changeTracking"),
            ...
            setSpanAttributes(span, {
                ...
                "scrape.index_hit": result.success && result.document.metadata.cacheState === "hit",
            });
            return result;
        }
        catch (error) {
            ...
            if (error instanceof NoEnginesLeftError) {
                ...
                meta.logger.warn("scrapeURL: Failed to prefetch document that is protected by anti-bot", { error });
            }
