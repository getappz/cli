import { generateCompletions, generateSchemaFromPrompt, } from "../transformers/llmExtract";
...
const hasRecursiveRefs = (schema, defs) => {
...
const containsRecursiveRef = (obj, targetDefName, defs, visited = new Set()) => {
    ...
    if (obj.$ref && typeof obj.$ref === "string") {
        ...
        if (refPath[0] === "#" && refPath[1] === "$defs") {
            ...
            if (defs[defName]) {
                const isRecursive = containsRecursiveRef(defs[defName], targetDefName, defs, visited);
                visited.delete(objKey);
                return isRecursive;
            }
        }
...
const resolveRefs = (obj, defs, logger, visited = new WeakSet(), depth = 0) => {
    if (!obj || typeof obj !== "object" || depth > 10)
        return obj;
    if (visited.has(obj)) {
        logger.warn("resolveRefs: Detected circular reference, aborting to prevent infinite recursion");
        return obj;
    }
    visited.add(obj);
    if (obj.$ref && typeof obj.$ref === "string") {
...
export async function extractData({ extractOptions, urls, useAgent, extractId, sessionId, scrapeId, metadata, }) {
    ...
    if (schema) {
        ...
        const hasAnyRefs = schema.$defs ||
            schemaString.includes('"$ref"') ||
            schemaString.includes("#/$defs/");
        if (hasAnyRefs) {
            logger.info("Detected schema with references, attempting to resolve refs", {
            ...
            try {
                ...
                const hasRemainingRefs = resolvedString.includes('"$ref"') ||
    ...
    try {
        ...
        if (useAgent && extract?.shouldUseSmartscrape) {
            ...
            const markdowns = await Promise.all(htmls.map(async (html) => await parseMarkdown(html, {
                logger,
                requestId,
            })));
