...
export const htmlTransform = async (html, url, scrapeOptions) => {
    let omceSignatures = undefined;
    if (scrapeOptions.__experimental_omce) {
        try {
            const hostname = scrapeOptions.__experimental_omceDomain || new URL(url).hostname;
