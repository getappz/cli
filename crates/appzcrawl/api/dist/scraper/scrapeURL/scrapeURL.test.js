...
describe("Standalone scrapeURL tests", () => {
    describe.each(testEngines)("Engine %s", (forceEngine) => {
        it("Basic scrape", async () => {
            const out = await scrapeURL("test:scrape-basic", "https://www.roastmywebsite.ai/", scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new CostTracking());
        ...
        it("Scrape with formats markdown and html", async () => {
            ...
            }), { forceEngine, teamId: "test" }, new CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 400 status code", async () => {
            const out = await scrapeURL("test:scrape-400", "https://httpstat.us/400", scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 401 status code", async () => {
            const out = await scrapeURL("test:scrape-401", "https://httpstat.us/401", scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 403 status code", async () => {
            const out = await scrapeURL("test:scrape-403", "https://httpstat.us/403", scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 404 status code", async () => {
            const out = await scrapeURL("test:scrape-404", "https://httpstat.us/404", scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 405 status code", async () => {
            const out = await scrapeURL("test:scrape-405", "https://httpstat.us/405", scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape of a page with 500 status code", async () => {
            const out = await scrapeURL("test:scrape-500", "https://httpstat.us/500", scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
        ...
        it("Scrape a redirected page", async () => {
            const out = await scrapeURL("test:scrape-redirect", "https://scrapethissite.com/", scrapeOptions.parse({}), { forceEngine, teamId: "test" }, new CostTracking());
            // expect(out.logs.length).toBeGreaterThan(0);
            expect(out.success).toBe(true);
            if (out.success) {
                expect(out.document.warning).toBeUndefined();
    ...
    it("Scrape of a PDF file", async () => {
        const out = await scrapeURL("test:scrape-pdf", "https://arxiv.org/pdf/astro-ph/9301001.pdf", scrapeOptions.parse({}), { teamId: "test" }, new CostTracking());
        // expect(out.logs.length).toBeGreaterThan(0);
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
    ...
    it("Scrape a DOCX file", async () => {
        const out = await scrapeURL("test:scrape-docx", "https://nvca.org/wp-content/uploads/2019/06/NVCA-Model-Document-Stock-Purchase-Agreement.docx", scrapeOptions.parse({}), { teamId: "test" }, new CostTracking());
        // expect(out.logs.length).toBeGreaterThan(0);
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
    ...
    it("Scrapes a XLSX file", async () => {
        const out = await scrapeURL("test:scrape-xlsx", "https://download.microsoft.com/download/1/4/E/14EDED28-6C58-4055-A65C-23B4DA81C4DE/Financial%20Sample.xlsx", scrapeOptions.parse({}), { teamId: "test" }, new CostTracking());
        // expect(out.logs.length).toBeGreaterThan(0);
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
    ...
    it("Sitemap scrape should not convert to markdown", async () => {
        const mockParseMarkdown = parseMarkdown;
        mockParseMarkdown.mockClear();
        const out = await scrapeURL("test:sitemap-no-markdown", "https://www.scrapethissite.com/sitemap.xml", scrapeOptions.parse({
            formats: ["rawHtml"],
        }), { teamId: "sitemap" }, new CostTracking());
        expect(out.success).toBe(true);
        if (out.success) {
            expect(out.document.warning).toBeUndefined();
