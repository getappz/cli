...
export async function deriveDiff(meta, document) {
    const changeTrackingFormat = hasFormatOfType(meta.options.formats, "changeTracking");
    if (changeTrackingFormat) {
        ...
        if (data && job) {
            ...
            const transformer = (x) => [...x.replace(/\s+/g, "").replace(/\[iframe\]\(.+?\)/g, "")]
                .sort()
                .join("");
            const isChanged = transformer(previousMarkdown) !== transformer(currentMarkdown);
            const changeStatus = document.metadata.statusCode === 404
                ? "removed"
                : isChanged
                    ? "changed"
                    : "same";
            ...
            if (changeTrackingFormat?.modes?.includes("git-diff") &&
            ...
            if (changeTrackingFormat?.modes?.includes("json") &&
                changeStatus === "changed") {
                try {
