...
async function deriveHTMLFromRawHTML(meta, document) {
    ...
    document.html = await htmlTransform(document.rawHtml, document.metadata.url ??
        document.metadata.sourceURL ??
        meta.rewrittenUrl ??
        meta.url, meta.options);
    return document;
}
async function deriveMarkdownFromHTML(meta, document) {
    ...
    if (document.metadata.postprocessorsUsed?.length && document.markdown) {
        meta.logger.debug("Skipping markdown derivation - postprocessor already set markdown", { postprocessorsUsed: document.metadata.postprocessorsUsed });
        return document;
    }
...
async function deriveImagesFromHTML(meta, document) {
    // Only derive if the formats has images
    if (hasFormatOfType(meta.options.formats, "images")) {
        if (document.html === undefined) {
            throw new Error("html is undefined -- this transformer is being called out of order");
        }
        document.images = await extractImages(document.html, document.metadata.url ??
            document.metadata.sourceURL ??
            meta.rewrittenUrl ??
            meta.url);
    }
...
async function deriveBrandingFromActions(meta, document) {
     ...
     */
    const brandingReturnIndex = document.actions?.javascriptReturns?.findIndex(x => x.type === "object" && "branding" in x.value);
    if (brandingReturnIndex === -1 || brandingReturnIndex === undefined) {
        return document;
    }
    // cast as any since this is js return, we might need to validate this
    const javascriptReturn = document.actions.javascriptReturns[brandingReturnIndex].value;
...
function coerceFieldsToFormats(meta, document) {
    ...
    if (!hasImages && document.images !== undefined) {
        meta.logger.warn("Removed images from Document because it wasn't in formats -- this is wasteful and indicates a bug.", { hasImages, hasImagesField: document.images !== undefined });
        delete document.images;
    }
    else if (hasImages && document.images === undefined) {
        meta.logger.warn("Request had format: images, but there was no images field in the result.", { hasImages, hasImagesField: document.images !== undefined });
    }
    ...
    if (!hasJson &&
        ...
        if (!shouldKeepJson && document.json !== undefined) {
            meta.logger.warn("Removed json from Document because it wasn't in formats -- this is extremely wasteful and indicates a bug.");
        ...
        meta.logger.warn("Request had format json, but there was no json field in the result.");
    }
    if (!hasSummary && document.summary !== undefined) {
        meta.logger.warn("Removed summary from Document because it wasn't in formats -- this is wasteful and indicates a bug.");
        delete document.summary;
    }
    else if (hasSummary && document.summary === undefined) {
        meta.logger.warn("Request had format summary, but there was no summary field in the result.");
    }
    if (!hasBranding && document.branding !== undefined) {
        meta.logger.warn("Removed branding from Document because it wasn't in formats -- this indicates the engine returned unexpected data.");
        delete document.branding;
    }
    else if (hasBranding && document.branding === undefined) {
        meta.logger.warn("Request had format branding, but there was no branding field in the result.");
    }
    ...
    if (meta.options.actions === undefined || meta.options.actions.length === 0) {
        ...
        const hasScreenshots = document.actions.screenshots && document.actions.screenshots.length > 0;
        const hasScrapes = document.actions.scrapes && document.actions.scrapes.length > 0;
        const hasJsReturns = document.actions.javascriptReturns &&
