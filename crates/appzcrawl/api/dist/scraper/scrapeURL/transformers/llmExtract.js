...
import { generateObject, generateText, NoObjectGeneratedError, jsonSchema, } from "ai";
...
function detectRecursiveSchema(schema) {
    if (!schema || typeof schema !== "object")
        return false;
    const schemaString = JSON.stringify(schema);
    const hasRefs = schemaString.includes('"$ref"') ||
...
function selectModelForSchema(schema) {
    if (!schema) {
...
export function trimToTokenLimit(text, maxTokens, modelId = "gpt-4o-mini", previousWarning) {
    try {
...
export async function generateCompletions({ logger, options, markdown, previousWarning, isExtractEndpoint, model = getModel("gpt-4o-mini", "openai"), mode = "object", providerOptions, retryModel = getModel("gpt-4.1", "openai"), costTrackingOptions, metadata, }) {
    let extract;
    let warning;
    let currentModel = model;
    let lastError = null;
    let modelId = typeof currentModel === "string" ? currentModel : currentModel.modelId;
        ...
        if (mode === "no-object") {
            try {
                ...
                costTrackingOptions.costTracking.addCall({
                    ...
                    model: modelId,
                    cost: calculateCost(modelId, result.usage?.inputTokens ?? 0, result.usage?.outputTokens ?? 0),
                    tokens: {
            ...
            experimental_repairText: async ({ text, error }) => {
                ...
                try {
                    ...
                    costTrackingOptions.costTracking.addCall({
                        ...
                        cost: calculateCost(modelId, repairUsage?.inputTokens ?? 0, repairUsage?.outputTokens ?? 0),
                        model: modelId,
                        tokens: {
...
export async function performLLMExtract(meta, document) {
    ...
    if (jsonFormat) {
        ...
        const { extractedDataArray, warning, costLimitExceededTokenUsage } = await extractData({
        ...
        meta.logger.debug("Assigning extracted data", {
            ...
            assigningTo: meta.internalOptions.v1OriginalFormat === "extract"
                ? "extract"
...
export async function performSummary(meta, document) {
    if (hasFormatOfType(meta.options.formats, "summary")) {
        ...
        const trimOutput = trimToTokenLimit(document.markdown, 120000, "gpt-4o-mini", document.warning);
        ...
        const { extract, warning, totalUsage, model } = await generateCompletions(generationOptions);
        if (warning) {
            document.warning =
                warning + (document.warning ? " " + document.warning : "");
        }
...
export async function generateSchemaFromPrompt(prompt, logger, costTracking, metadata) {
...
export async function generateCrawlerOptionsFromPrompt(prompt, logger, costTracking, metadata) {
    const model = getModel("gpt-4o-mini", "openai");
    const retryModel = getModel("gpt-4.1", "openai");
...
Return a JSON object with only the relevant options for the user's request. Don't include options that aren't relevant to the instruction. Focus on the most important options that directly address the user's intent.`,
    ...
    throw new Error(`Failed to generate crawler options after all attempts. Last error: ${lastError?.message}`);
}
