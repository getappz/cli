 ...
 */
function shouldIndexForSearch(meta, document) {
 ...
 * Transformer: Send document to search index
 */
export async function sendDocumentToSearchIndex(meta, document) {
    // Check if search indexing is enabled via the SEARCH_SERVICE_URL
    const searchIndexEnabled = config.ENABLE_SEARCH_INDEX && config.SEARCH_SERVICE_URL;
    ...
    (async () => {
        try {
            await indexDocumentIfEnabled({
                url: meta.url,
                resolvedUrl: document.metadata.url ??
                    document.metadata.sourceURL ??
                    meta.rewrittenUrl ??
                    meta.url,
                title: document.metadata.title ?? document.metadata.ogTitle ?? undefined,
...
//# sourceMappingURL=sendToSearchIndex.js.map