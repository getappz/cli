...
export function calculateScrapeCredits(input) {
 ...
 */
export async function billTeam(db, params) {
    ...
    const batch = [
        ...
        db
            .prepare(`INSERT INTO billing_log (id, team_id, job_id, job_type, credits, reason, balance_after, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
            .bind(logId, teamId, jobId ?? null, jobType ?? null, credits, reason, newBalance, now),
    ];
 ...
 */
export async function refundCredits(db, params) {
    const { teamId, credits, jobId, jobType, reason } = params;
    const logId = crypto.randomUUID();
    const now = new Date().toISOString();
    const batch = [
        db
            .prepare("UPDATE team_credits SET credits = credits + ? WHERE team_id = ?")
            .bind(credits, teamId),
        db
            .prepare(`INSERT INTO billing_log (id, team_id, job_id, job_type, credits, reason, balance_after, created_at)
         VALUES (?, ?, ?, ?, ?, ?, NULL, ?)`)
            .bind(logId, teamId, jobId ?? null, jobType ?? null, -credits, reason, now),
    ];
    await db.batch(batch);
}
