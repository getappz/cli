 ...
 */
export async function checkCrawlCompletion(db, crawlId) {
 ...
 */
export async function checkAllCrawlCompletions(db) {
...
export async function getCrawlProgress(db, crawlId) {
    const crawl = await getCrawlJob(db, crawlId);
    if (!crawl)
        return null;
    const percentage = crawl.totalCount > 0
...
//# sourceMappingURL=crawl-completion-checker.js.map