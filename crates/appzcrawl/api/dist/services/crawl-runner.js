...
import { addRobotsBlocked, getCrawlJob, isCrawlCancelled, setTotalCount, updateCrawlStatus, } from "./crawl-store";
import { filterLinks } from "./html-processor";
import { USER_AGENT } from "./scrape-fetcher";
import { buildScrapeOptsForQueue, buildScrapeRunnerOptions, } from "./scrape-options";
...
function normalizeUrlForCrawlDedup(url) {
    try {
        const u = new URL(url);
...
async function discoverLinks(params) {
    const { env, rawLinks, seen, queue, limit, currentCount, crawl, maxDepth, baseUrl, robotsTxt, } = params;
...
export async function runCrawl(env, seedUrl, options = {}) {
    ...
    const errors = [];
    const seen = new Set();
    const queue = [normalizeUrlForCrawlDedup(seedUrl)];
    while (queue.length > 0 && data.length < limit) {
        ...
        if (batch.length === 0)
            continue;
        const results = await Promise.allSettled(batch.map((url) => runScrapeUrl(env, url, { onlyMainContent })));
        for (let i = 0; i < results.length; i++) {
            ...
            for (const link of doc.links) {
                ...
                if (seen.has(linkNorm))
                    continue;
                if (sameOriginOnly && !sameOrigin(seedUrl, trimmed))
                    continue;
                if (!isValidUrl(trimmed))
                    continue;
                if (queue.length + data.length >= limit)
                    break;
                queue.push(linkNorm);
            }
        }
...
export async function runCrawlAsync(env, crawlId) {
    ...
    const seen = new Set();
    const queue = [normalizeUrlForCrawlDedup(crawl.originUrl)];
    let enqueuedCount = 0;
...
async function processFanOutBatch(env, params) {
    const { crawlId, crawl, queue, seen, limit, scrapeOpts, scrapeOptsForQueue, maxDepth, baseUrl, robotsTxt, } = params;
    ...
    if (urlBatch.length === 0)
        return;
    const messages = urlBatch.map((url) => ({
        body: {
            jobType: "crawl",
            crawlId,
            url,
...
async function processInlineBatch(env, params) {
    const { crawlId, crawl, queue, seen, limit, concurrency, scrapeOpts, maxDepth, baseUrl, robotsTxt, opts, } = params;
    let { completedInRunner } = params;
    const batchSize = Math.min(concurrency, limit - completedInRunner, queue.length);
    const batch = [];
    for (let i = 0; i < batchSize; i++) {
        const url = queue.shift();
        if (!url)
    ...
    const results = await Promise.allSettled(batch.map((url) => runScrapeUrl(env, url, scrapeOpts)));
    for (let i = 0; i < results.length; i++) {
        const url = batch[i];
        const result = results[i];
        if (result.status === "rejected") {
