...
import { checkAndUpdateURLForMap, isSameDomain, isSameSubdomain, removeDuplicateUrls, } from "../lib/validateUrl";
...
async function fetchSitemapXml(env, sitemapUrl, engine) {
...
async function collectFromSitemap(env, sitemapUrl, out, seenSitemaps, limit, _timeoutMs, engine) {
    if (seenSitemaps.size >= SITEMAP_LIMIT || out.length >= limit)
        return;
    ...
    for (const child of sitemapUrls) {
        if (out.length >= limit || seenSitemaps.size >= SITEMAP_LIMIT)
            break;
        await collectFromSitemap(env, child, out, seenSitemaps, limit, _timeoutMs, engine);
    }
}
/** Score for search relevance (higher = better match). Simple: URL/path contains query words. */
function searchRelevance(url, query) {
 ...
 */
function patternToRegex(pattern, regexOnFullURL) {
    if (regexOnFullURL) {
 ...
 */
function matchesPathPatterns(url, includePaths, excludePaths, regexOnFullURL) {
...
export async function getMapResults(env, options) {
    ...
    if (sitemapMode === "only" || sitemapMode === "include") {
        ...
        for (const sitemapUrl of candidates) {
            if (links.length >= limit)
                break;
            await collectFromSitemap(env, sitemapUrl, links, seenSitemaps, limit, timeoutMs, engine);
        }
    }
    if (sitemapMode === "skip" ||
    ...
    for (const doc of links) {
        try {
            const { url: normalized } = checkAndUpdateURLForMap(doc.url, ignoreQueryParameters);
    ...
    if (includePaths.length > 0 || excludePaths.length > 0) {
        mapped = mapped.filter((x) => matchesPathPatterns(x.url, includePaths, excludePaths, regexOnFullURL));
    }
    // Deduplication
    ...
    if (options.search) {
        ...
        final = [...final].sort((a, b) => searchRelevance(b.url, searchQuery) -
...
//# sourceMappingURL=map-runner.js.map