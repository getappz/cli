 ...
 */
export async function parsePdfWithSarvam(apiKey, pdfData, opts = {}) {
    const { timeoutMs = 180_000, language = "en-IN", requestedFormats = ["md"], } = opts;
    ...
    const runOneJob = async (outputFormat) => {
        const headers = {
        ...
        if (!createRes.ok) {
            ...
            throw new Error(`Sarvam create job failed: ${createRes.status} ${errBody}`);
        }
        const createJson = (await createRes.json());
        const jobId = createJson.job_id;
        if (!jobId || typeof jobId !== "string") {
            throw new Error(`Sarvam create response missing job_id: ${JSON.stringify(createJson)}`);
        }
        const uploadLinksRes = await fetch(`${SARVAM_BASE}/doc-digitization/job/v1/upload-files`, {
            method: "POST",
            headers,
        ...
        if (!uploadLinksRes.ok) {
            ...
            throw new Error(`Sarvam upload links failed: ${uploadLinksRes.status} ${errBody}`);
        }
        const uploadJson = (await uploadLinksRes.json());
        const firstEntry = uploadJson.upload_urls && Object.values(uploadJson.upload_urls)[0];
        const fileUrl = firstEntry?.file_url;
        if (!fileUrl) {
            throw new Error(`Sarvam upload URLs invalid: ${JSON.stringify(uploadJson)}`);
        }
        const uploadHeaders = {
        ...
        if (!uploadRes.ok) {
            throw new Error(`Sarvam file upload failed: ${uploadRes.status} ${uploadRes.statusText}`);
        }
        const startRes = await fetch(`${SARVAM_BASE}/doc-digitization/job/v1/${encodeURIComponent(jobId)}/start`, { method: "POST", headers: { "api-subscription-key": apiKey }, signal });
        if (!startRes.ok) {
        ...
        for (let i = 0; i < MAX_POLL_ATTEMPTS; i++) {
            if (signal.aborted)
                throw new Error("Sarvam aborted");
            const statusRes = await fetch(`${SARVAM_BASE}/doc-digitization/job/v1/${encodeURIComponent(jobId)}/status`, { headers: { "api-subscription-key": apiKey }, signal });
            if (!statusRes.ok) {
        ...
        const downloadRes = await fetch(`${SARVAM_BASE}/doc-digitization/job/v1/${encodeURIComponent(jobId)}/download-files`, { method: "POST", headers: { "api-subscription-key": apiKey }, signal });
        if (!downloadRes.ok) {
            const errBody = await downloadRes.text();
            throw new Error(`Sarvam download links failed: ${downloadRes.status} ${errBody}`);
        }
        const downloadJson = (await downloadRes.json());
        const dlEntry = downloadJson.download_urls &&
            Object.values(downloadJson.download_urls)[0];
        const dlUrl = dlEntry?.file_url;
        if (!dlUrl) {
            throw new Error(`Sarvam download URLs invalid: ${JSON.stringify(downloadJson)}`);
        }
        const zipRes = await fetch(dlUrl, { signal });
        if (!zipRes.ok) {
            throw new Error(`Sarvam download failed: ${zipRes.status} ${zipRes.statusText}`);
        }
        ...
        const entry = Object.entries(unzipped).find(([name]) => name.toLowerCase().endsWith(ext));
        if (!entry) {
            throw new Error(`Sarvam output ZIP has no ${ext}: ${Object.keys(unzipped).join(", ")}`);
        }
        const [, data] = entry;
...
//# sourceMappingURL=sarvam-client.js.map