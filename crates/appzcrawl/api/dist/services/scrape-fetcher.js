 ...
 */
import { resolveScreenshotOptions, } from "../contracts/scrape";
import { logger } from "../lib/logger";
import { uploadScreenshotToR2 } from "../lib/screenshot-upload";
import { cloudflareFetchContent, cloudflareFetchSnapshot, isCloudflareBrowserEnabled, } from "./cloudflare-browser-client";
...
// Constants
// ---------------------------------------------------------------------------
export const USER_AGENT = "Mozilla/5.0 (compatible; Appzcrawl/1.0; +https://appz.dev)";
...
export function isDocumentUrl(url) {
    ...
    return DOCUMENT_EXTENSIONS.some((ext) => lower.endsWith(ext) || lower.includes(`${ext}/`));
}
export function isPdfUrl(url) {
    const lower = url.toLowerCase();
    return PDF_EXTENSIONS.some((ext) => lower.endsWith(ext) || lower.includes(`${ext}/`));
}
function isValidDocumentContentType(contentType) {
    if (!contentType)
        return false;
...
export function resolveEngine(env, options) {
    const engine = options.engine;
    if ((engine === "cloudflare" || engine === "auto") &&
...
// ---------------------------------------------------------------------------
export async function fetchHtmlWithWorker(url, opts = {}) {
...
// ---------------------------------------------------------------------------
export async function fetchDocumentWithWorker(url, opts = {}) {
    const result = await fetchWithTimeout(url, opts.timeout ?? 30_000);
    if ("success" in result)
        return result;
    const { res } = result;
...
// ---------------------------------------------------------------------------
export async function fetchPdfWithWorker(url, opts = {}) {
    const result = await fetchWithTimeout(url, opts.timeout ?? 30_000);
    if ("success" in result)
        return result;
    const { res } = result;
...
// ---------------------------------------------------------------------------
export async function captureScreenshotAndUpload(env, url, opts) {
    const { formats, screenshotOptions, screenshotBaseUrl, mobile = false, blockAds = true, timeout: screenshotTimeout = 30_000, waitFor = 0, } = opts;
    const resolved = resolveScreenshotOptions(formats, screenshotOptions);
    if (!resolved || !screenshotBaseUrl || !env.BROWSER_SERVICE)
        return undefined;
...
// ---------------------------------------------------------------------------
async function tryPdfSarvamOrNative(env, pdfData, effectiveTimeout, formats) {
...
function resolveSarvamFormats(formats) {
    const out = [];
...
export async function fetchContent(env, url, options) {
    const { resolvedEngine, useFireEngine, effectiveTimeout, skipTlsVerification, mobile, wantsScreenshot, screenshotBaseUrl, screenshotOptions, formats, } = options;
    ...
    if (isPdfUrl(url)) {
        ...
        if (llamaKey && llamaKey.trim().length > 0) {
            try {
                ...
                logger.warn("[scrape] LlamaParse failed, falling back to Sarvam/native", {
                    error: e instanceof Error ? e.message : String(e),
                    url,
                });
                const fallback = await tryPdfSarvamOrNative(env, pdfResult.data, effectiveTimeout, formats);
                html = fallback.html;
            ...
            try {
                ...
                logger.warn("[scrape] Sarvam failed, falling back to native pdf-extract", {
                    error: e instanceof Error ? e.message : String(e),
                    url,
                });
    ...
    const tryFireEngine = useFireEngine && isFireEngineEnabled(env) && fireEngineUrl;
    async function tryCloudflareFetch() {
        ...
        if (wantsScreenshot && screenshotBaseUrl && env.BUCKET) {
            ...
            let screenshotUrl;
            try {
                const binary = Uint8Array.from(atob(snap.screenshotBase64), (c) => c.charCodeAt(0));
                const { filename } = await uploadScreenshotToR2(env.BUCKET, binary, "image/png");
        ...
        const content = await cloudflareFetchContent(accountId, apiToken, url, cfOptions);
        if (!content.success)
...
//# sourceMappingURL=scrape-fetcher.js.map