 ...
 */
import { getAssetFormatsToExtract, } from "../contracts/scrape";
import { removeBase64ImagesFromMarkdown } from "../lib/removeBase64Images";
import { extractAssets, extractImages, htmlToMarkdown, postProcessMarkdown, } from "./html-processor";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
/** Flatten an ExtractedAssetsResult into a single string array. */
export function flattenAssets(result) {
    return [
        ...(result.images ?? []),
        ...(result.css ?? []),
        ...(result.js ?? []),
 ...
 */
export async function processFormats(env, html, rawHtml, url, formats, citations, removeBase64Images) {
    ...
    const wantsMarkdown = formats.includes("markdown") || formats.includes("json");
    ...
    if (wantsAnyAssetExtraction) {
        try {
            ...
            assetsResult = await extractAssets(env, htmlForExtract, url, assetFormatsToExtract);
...
//# sourceMappingURL=scrape-format-processor.js.map