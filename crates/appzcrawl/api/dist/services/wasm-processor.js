...
import { compute_engpicker_verdict, convert_document, extract_assets, extract_attributes, extract_base_href, extract_images, extract_links, extract_metadata, filter_links, filter_url, get_inner_json, initSync, parse_sitemap_xml, post_process_markdown, process_sitemap, transform_html, } from "../wasm/firecrawl_wasm.js";
...
function detectDocType(url, contentType) {
    ...
    if (ct.includes("application/vnd.openxmlformats-officedocument.wordprocessingml"))
        return "docx";
...
export function getWasmDiagnostics() {
    return {
        ...
        is_wasm_module: typeof WebAssembly !== "undefined" &&
...
export function extractLinks(html) {
    const result = JSON.parse(extract_links(html ?? ""));
    return { links: result };
}
export function extractBaseHref(html, baseUrl) {
    const result = JSON.parse(extract_base_href(html, baseUrl));
...
export function transformHtml(params) {
    const opts = {
        html: params.html,
        url: params.url,
    ...
    const result = JSON.parse(transform_html(JSON.stringify(opts)));
    return { html: result };
}
export function getInnerJson(html) {
    const result = JSON.parse(get_inner_json(html));
    return { content: result };
}
export function extractAttributes(html, options) {
    const results = JSON.parse(extract_attributes(html, JSON.stringify({ selectors: options.selectors })));
    return { results };
}
export function extractImages(html, baseUrl) {
    const result = JSON.parse(extract_images(html, baseUrl));
    return { images: result };
}
export function extractAssets(html, baseUrl, formats) {
    const result = JSON.parse(extract_assets(html, baseUrl, JSON.stringify(formats ?? ["assets"])));
    return result;
}
export function postProcessMarkdown(markdown, _options) {
    ...
    const result = JSON.parse(post_process_markdown(markdown));
    return { markdown: result };
}
export function filterLinks(params) {
    const body = {
        links: params.links,
...
export function convertDocumentToHtml(params) {
    ...
    if (!docType) {
        throw new Error(`WASM: unable to detect document type from url=${params.url} contentType=${params.contentType}`);
    }
    // Encode to base64
    ...
    const result = JSON.parse(convert_document(dataBase64, docType));
    return { html: result };
}
...
//# sourceMappingURL=wasm-processor.js.map