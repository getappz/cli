 ...
 */
export async function deliverWebhook(db, params) {
    ...
    if (params.hmacSecret) {
        headers["X-Firecrawl-Signature"] = await signPayload(body, params.hmacSecret);
    }
    let statusCode = null;
    let success = false;
    ...
    try {
        ...
        await db
            .prepare(`INSERT INTO webhook_logs
          (id, team_id, job_id, job_type, event, url, status_code, success, attempt, request_body, response_body, error, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?)`)
            .bind(logId, params.teamId, params.jobId, params.jobType, params.event, params.url, statusCode, success ? 1 : 0, 
        // Don't store full request body for large payloads (truncate to 1KB)
        body.length > 1000 ? `${body.slice(0, 1000)}...` : body, responseBody, error, now)
            .run();
    }
    catch (e) {
        // Best-effort logging
 ...
 */
export async function sendCrawlWebhook(db, params) {
