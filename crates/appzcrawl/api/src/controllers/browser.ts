...
import {
  ...
  parseBrowserDeleteRequest,
  parseBrowserExecuteRequest,
} from "../contracts/browser";
import { logger } from "../lib/logger";
import type { AppEnv } from "../types";

// ============================================================================
// Shared helpers (DRY auth + service check + body parse)
// ============================================================================

interface ControllerContext {
  auth: { team_id: string };
  browserService: NonNullable<AppEnv["Bindings"]["BROWSER_SERVICE"]>;
}

/** Validate auth + browser service availability. Returns null + sends error response on failure. */
function getControllerContext(c: Context<AppEnv>): ControllerContext | null {
...
async function parseBody(c: Context<AppEnv>): Promise<unknown> {
  try {
...
function sessionErrorStatus(error: string): 403 | 404 | 410 | 500 {
...
export async function browserCreateController(c: Context<AppEnv>) {
...
export async function browserExecuteController(c: Context<AppEnv>) {
  const ctx = getControllerContext(c);
  if (!ctx) return c.res;
      ...
      return c.json(
        {
          success: false,
          error:
            "Python code execution requires sandbox environment. Use 'actions' with 'evaluate' for JavaScript.",
        },
        400,
      );
    }
    actionsToExecute = [{ action: "evaluate" as const, code }];
...
export async function browserDeleteController(c: Context<AppEnv>) {
  const ctx = getControllerContext(c);
  if (!ctx) return c.res;
...
export async function browserStatusController(c: Context<AppEnv>) {
  const ctx = getControllerContext(c);
  if (!ctx) return c.res;
  ...
  if (!browserId) {
    return c.json(
      { success: false, error: "Missing browserId parameter" },
      400,
    );
  }
  ...
  try {
    ...
    if (!result.success) {
      return c.json(
        ...
        sessionErrorStatus(result.error),
      );
    }
    ...
    const ttlRemaining = Math.max(
      0,
      session.ttlTotal - Math.floor((now - session.createdAt) / 1000),
    );
    ...
    return c.json(
      {
        success: false,
        error:
          err instanceof Error ? err.message : "Failed to get browser session",
      },
      500,
    );
  }
