...
export async function extractController(c: Context<AppEnv>) {
  ...
  const systemPrompt =
    typeof rawBody.systemPrompt === "string" ? rawBody.systemPrompt : undefined;
  const webhook =
  ...
  if (!urls?.length && !prompt) {
    return c.json(
      { success: false, error: "Either 'urls' or 'prompt' is required" },
      400,
    );
  }
  ...
  try {
    ...
    return c.json(
      { success: false, error: "Failed to create extract job" },
      500,
    );
  }
...
export async function extractStatusController(c: Context<AppEnv>) {
