...
export default {
   ...
   */
  async queue(
    batch: MessageBatch<CrawlQueueMessage | ScrapeQueueMessage>,
    env: AppEnv["Bindings"],
  ): Promise<void> {
    // Route to appropriate consumer based on queue name
    if (batch.queue === "appzcrawl-scrape-queue") {
      await processScrapeQueue(batch as MessageBatch<ScrapeQueueMessage>, env);
    } else {
      // Default to crawl queue (appzcrawl-crawl-queue)
      await processCrawlQueue(batch as MessageBatch<CrawlQueueMessage>, env);
    }
  },
