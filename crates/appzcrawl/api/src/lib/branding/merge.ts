...
export function mergeBrandingResults(
  ...
  if (
    ...
    if (llm.logoSelection.selectedLogoIndex === -1) {
      ...
      (merged as Record<string, unknown>).__llm_logo_reasoning = {
        selectedIndex: -1,
        reasoning:
      ...
      if (selectedLogo) {
        ...
        if (href?.trim()) {
          ...
          if (
            ...
            if (
              externalServiceDomains.some((domain) =>
                hrefLower.includes(domain),
              )
            ) {
              isExternalLink = true;
            }
  ...
  if (buttonSnapshots.length > 0) {
    ...
    (merged as Record<string, unknown>).__llm_button_reasoning = {
      primary: {
        index: primaryIdx,
  ...
  if (llm.personality) {
    (merged as Record<string, unknown>).personality = llm.personality;
  }

  if (llm.designSystem) {
    (merged as Record<string, unknown>).designSystem = llm.designSystem;
  }
  ...
  if (llm.cleanedFonts && llm.cleanedFonts.length > 0) {
    ...
    const cleanFontName = (font: string): string => {
      ...
      for (const cleanedFont of llm.cleanedFonts) {
        ...
        if (fontLower === cleanedLower) return cleanedFont.family;
        if (fontLower.includes(cleanedLower)) return cleanedFont.family;
    ...
    if (
      ...
      const stacks = merged.typography.fontStacks as Record<string, string[]>;
      const cleanStack = (
  ...
  (merged as Record<string, unknown>).confidence = {
