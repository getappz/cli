...
function inferPalette(
  ...
  if (pageBackground) {
    ...
    if (pageBgHex) bump(pageBgHex, 1000);
  }
...
function extractInputSnapshots(
  ...
  const results: InputSnapshot[] = [];
  for (const { input } of topInputs) {
    if (!input.inputMetadata) continue;
    const meta = input.inputMetadata;
...
export function processRawBranding(raw: BrandingScriptReturn): BrandingProfile {
  ...
  const candidateButtons = raw.snapshots
    ...
    .sort(
      (a: { _score?: number }, b: { _score?: number }) =>
  ...
  const buttonSnapshots = topButtons.map((s, idx) => {
    ...
    const corners = {
      topLeft:
        s.borderRadius?.topLeft != null ? `${s.borderRadius.topLeft}px` : "0px",
      topRight:
  ...
  return {
    ...
    components,
    images,
    __button_snapshots: buttonSnapshots as unknown,
    __input_snapshots: inputSnapshots as unknown,
    __framework_hints: raw.frameworkHints as unknown,
    __logo_candidates: raw.logoCandidates as unknown,
  };
}
