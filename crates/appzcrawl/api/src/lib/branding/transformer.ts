...
export interface BrandingTransformerInput {
  ...
  aiBinding?: { run: (model: string, options: object) => Promise<unknown> };
}
...
export async function brandingTransformer(
  input: BrandingTransformerInput,
  ...
  const buttonSnapshots: ButtonSnapshot[] =
    (jsBranding.__button_snapshots as ButtonSnapshot[]) ?? [];
  ...
  try {
    ...
    const buttonIndexMap = new Map<number, number>();
    ...
    if (buttonSnapshots.length > 12) {
      const scored = buttonSnapshots
        .map((btn, idx) => {
          ...
          if (primaryCtaKeywords.some((kw) => text.includes(kw))) score += 100;
          const secondaryCtaKeywords = ["try", "start", "view", "explore"];
          if (secondaryCtaKeywords.some((kw) => text.includes(kw))) score += 50;
          if (
            bgColor &&
            !bgColor.match(
    ...
    const llmEnhancement = await enhanceBrandingWithLLM(
      {
        ...
        logoCandidates:
          filteredCandidates.length > 0 ? filteredCandidates : undefined,
        brandName,
  ...
  if (!input.debugBranding) {
    delete (brandingProfile as Record<string, unknown>).__button_snapshots;
    delete (brandingProfile as Record<string, unknown>).__input_snapshots;
    delete (brandingProfile as Record<string, unknown>).__logo_candidates;
  }
