...
function shouldReadEdgeCache(body: ParsedBody): boolean {
  if (body.maxAge <= 0) return false;
  if (body.formats.includes("changeTracking")) return false;
  if (body.zeroDataRetention) return false;
  if (body.headers && Object.keys(body.headers).length > 0) return false;
  return true;
}

/** Checks whether we should write to edge cache after the response. */
function shouldWriteEdgeCache(body: ParsedBody): boolean {
  if (body.zeroDataRetention) return false;
  return true;
}
...
function parseBody(raw: unknown): ParsedBody | null {
  ...
  const formats = Array.isArray(obj.formats)
    ? (obj.formats as string[]).filter((f) => typeof f === "string")
  ...
  return {
    ...
    onlyMainContent:
      typeof obj.onlyMainContent === "boolean"
        ? obj.onlyMainContent
        : SCRAPE_DEFAULTS.onlyMainContent,
    includeTags: Array.isArray(obj.includeTags)
      ? (obj.includeTags as string[]).filter((t) => typeof t === "string")
      : [],
    excludeTags: Array.isArray(obj.excludeTags)
      ? (obj.excludeTags as string[]).filter((t) => typeof t === "string")
      : [],
    mobile:
      typeof obj.mobile === "boolean" ? obj.mobile : SCRAPE_DEFAULTS.mobile,
    removeBase64Images:
      typeof obj.removeBase64Images === "boolean"
        ? obj.removeBase64Images
        : SCRAPE_DEFAULTS.removeBase64Images,
    blockAds:
      typeof obj.blockAds === "boolean"
        ? obj.blockAds
    ...
    storeInCache:
      typeof obj.storeInCache === "boolean"
        ? obj.storeInCache
        : SCRAPE_DEFAULTS.storeInCache,
    zeroDataRetention:
      typeof obj.zeroDataRetention === "boolean"
...
async function buildEdgeCacheKey(
  requestUrl: string,
  body: ParsedBody,
): Promise<Request> {
  const canonicalUrl = normalizeUrlForCache(
    /^https?:\/\//i.test(body.url) ? body.url : `https://${body.url}`,
  );
 ...
 */
export const edgeCacheMiddleware: MiddlewareHandler<AppEnv> = async (
  c,
  next,
) => {
  const t0 = Date.now();
  ...
  if (res.status === 200 && shouldWriteEdgeCache(body)) {
    ...
    c.executionCtx.waitUntil(
      cache.put(cacheKeyReq, responseToCache).catch((e) => {
