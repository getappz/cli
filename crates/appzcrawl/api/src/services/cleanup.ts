...
export async function runScheduledCleanup(
  ...
  const results: Array<{ task: string; deleted: number; error?: string }> = [];
...
async function cleanExpiredJobsWithR2(
  db: D1Database,
  bucket: R2Bucket,
  table: "extract_jobs" | "agent_jobs",
): Promise<number> {
  const now = new Date().toISOString();

  const { results: rows } = await db
    .prepare(
      `SELECT id, r2_key FROM ${table}
...
async function cleanOldJobsWithR2(
  db: D1Database,
  bucket: R2Bucket,
  table: "scrapes",
  ttlMs: number,
...
async function deleteRowsWithR2(
  db: D1Database,
  bucket: R2Bucket,
  table: string,
  rows: Array<{ id: string; r2_key: string | null }>,
): Promise<number> {
  const r2Keys = rows.filter((r) => r.r2_key).map((r) => r.r2_key as string);
  if (r2Keys.length > 0) {
...
async function cleanExpiredCrawls(
  db: D1Database,
  bucket: R2Bucket,
): Promise<number> {
  const now = new Date().toISOString();
  ...
  for (const { id: crawlId } of expiredRows) {
    ...
    const { results: r2Rows } = await db
      .prepare(
        "SELECT r2_key FROM crawl_results WHERE crawl_id = ? AND r2_key IS NOT NULL",
      )
      .bind(crawlId)
      .all<{ r2_key: string }>();
...
async function cleanExpiredCache(
  ...
  const { results: rows } = await db
    .prepare(
      `SELECT id, r2_key FROM scrape_cache WHERE expires_at_ms < ? LIMIT ?`,
    )
...
async function cleanOldRows(
  db: D1Database,
  table: "webhook_logs" | "request_log" | "billing_log",
  ttlMs: number,
): Promise<number> {
  const cutoff = new Date(Date.now() - ttlMs).toISOString();
