 ...
 */
export function normalizeUrlForDedup(
  url: string,
...
export async function lockUrl(
  ...
  try {
    const result = await db
      .prepare(
        `INSERT OR IGNORE INTO crawl_visited_urls (crawl_id, url_hash, url, created_at)
         VALUES (?, ?, ?, ?)`,
      )
 ...
 */
export async function isUrlVisited(
  db: D1Database,
  crawlId: string,
  url: string,
  opts?: { ignoreQueryParameters?: boolean },
  ...
  const row = await db
    .prepare(
      "SELECT 1 FROM crawl_visited_urls WHERE crawl_id = ? AND url_hash = ? LIMIT 1",
    )
 ...
 */
export async function getVisitedCount(
  db: D1Database,
  crawlId: string,
): Promise<number> {
  const row = await db
    .prepare(
      "SELECT COUNT(*) as cnt FROM crawl_visited_urls WHERE crawl_id = ?",
    )
    .bind(crawlId)
    .first<{ cnt: number }>();
