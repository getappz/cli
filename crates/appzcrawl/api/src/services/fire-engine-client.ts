...
export type FireEngineFetchOutput =
 ...
 */
export async function fireEngineFetchHtml(
  baseUrl: string,
  url: string,
  options: { timeout?: number; skipTlsVerification?: boolean } = {},
  ...
  if (typeof data.error === "string") {
    return {
      ...
      error: data.error,
      statusCode:
        typeof data.pageStatusCode === "number"
          ? data.pageStatusCode
          : undefined,
    };
  }
    ...
    while (Date.now() < pollUntil) {
      ...
      if (typeof pollData.error === "string") {
        ...
        return {
          ...
          statusCode:
            typeof pollData.pageStatusCode === "number"
  ...
  if (
    ...
    const statusCode =
      typeof data.pageStatusCode === "number" ? data.pageStatusCode : 200;
    if (typeof content !== "string") {
      return {
        success: false,
        error: "Fire-engine returned non-string content",
...
async function deleteJob(base: string, jobId: string): Promise<void> {
  try {
