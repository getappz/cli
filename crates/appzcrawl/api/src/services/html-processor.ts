...
// ---------------------------------------------------------------------------
// Re-export types that callers may need (unchanged)
// ---------------------------------------------------------------------------
 ...
 */
function useContainer(env: AppEnv["Bindings"]): boolean {
  const flag = (env as Record<string, unknown>).USE_CONTAINER_BACKEND;
 ...
 */
function useWasm(env: AppEnv["Bindings"]): boolean {
  if (!wasmNative.isAvailable()) return false;
  const flag = (env as Record<string, unknown>).DISABLE_WASM_BACKEND;
  return flag !== "true" && flag !== true;
}
 ...
 */
function useEngine(env: AppEnv["Bindings"]): boolean {
...
export async function extractLinks(
  ...
  if (useContainer(env)) return container.extractLinks(env, html);
  if (useWasm(env)) return wasmNative.extractLinks(html);
  if (useEngine(env)) return engine.extractLinks(env, html);
...
export async function extractBaseHref(
  ...
  if (useContainer(env)) return container.extractBaseHref(env, html, baseUrl);
  if (useWasm(env)) return wasmNative.extractBaseHref(html, baseUrl);
  if (useEngine(env)) return engine.extractBaseHref(env, html, baseUrl);
...
export async function extractMetadata(
  ...
  if (useContainer(env)) return container.extractMetadata(env, html);
  if (useWasm(env)) return wasmNative.extractMetadata(html);
  if (useEngine(env)) return engine.extractMetadata(env, html);
...
export async function getInnerJson(
  ...
  if (useContainer(env)) return container.getInnerJson(env, html);
  if (useWasm(env)) return wasmNative.getInnerJson(html);
  if (useEngine(env)) return engine.getInnerJson(env, html);
...
export async function extractImages(
  ...
  if (useContainer(env)) return container.extractImages(env, html, baseUrl);
  if (useWasm(env)) return wasmNative.extractImages(html, baseUrl);
  if (useEngine(env)) return engine.extractImages(env, html, baseUrl);
...
export async function extractAssets(
  ...
  baseUrl: string,
  formats?: string[],
): Promise<container.ExtractedAssetsResult> {
  if (useContainer(env))
    return container.extractAssets(env, html, baseUrl, formats);
  if (useWasm(env)) return wasmNative.extractAssets(html, baseUrl, formats);
  if (useEngine(env)) return engine.extractAssets(env, html, baseUrl, formats);
  return container.extractAssets(env, html, baseUrl, formats);
}
...
export async function postProcessMarkdown(
  ...
  if (options?.citations)
    return container.postProcessMarkdown(env, markdown, options);
  if (useWasm(env)) return wasmNative.postProcessMarkdown(markdown, options);
  if (useEngine(env)) return engine.postProcessMarkdown(env, markdown, options);
  return container.postProcessMarkdown(env, markdown, options);
}
...
export async function parseSitemap(
  ...
  if (useContainer(env)) return container.parseSitemap(env, xml);
  if (useWasm(env)) return wasmNative.parseSitemap(xml);
  if (useEngine(env)) return engine.parseSitemap(env, xml);
...
export async function extractAttributes(
  env: AppEnv["Bindings"],
  html: string,
  options: { selectors: container.ExtractAttributesOption[] },
): Promise<{ results: unknown }> {
  if (useContainer(env)) return container.extractAttributes(env, html, options);
  if (useWasm(env)) return wasmNative.extractAttributes(html, options);
  if (useEngine(env)) return engine.extractAttributes(env, html, options);
  return container.extractAttributes(env, html, options);
}

export async function transformHtml(
  env: AppEnv["Bindings"],
  params: container.TransformHtmlParams,
): Promise<{ html: string }> {
  if (useContainer(env)) return container.transformHtml(env, params);
  if (useWasm(env)) return wasmNative.transformHtml(params);
  if (useEngine(env)) return engine.transformHtml(env, params);
  return container.transformHtml(env, params);
}

export async function filterLinks(
  env: AppEnv["Bindings"],
  params: container.FilterLinksParams,
): Promise<container.FilterLinksResult> {
  if (useContainer(env)) return container.filterLinks(env, params);
  if (useWasm(env)) return wasmNative.filterLinks(params);
  if (useEngine(env)) return engine.filterLinks(env, params);
...
export async function convertDocument(
  ...
  if (useContainer(env)) return container.convertDocument(env, params);
  if (useWasm(env)) {
    try {
...
export async function htmlToMarkdown(
  ...
  if (useEngine(env)) return engine.htmlToMarkdown(env, html);
...
export async function nativeSearch(
  env: AppEnv["Bindings"],
  options: container.NativeSearchOptions,
): Promise<container.NativeSearchResponse> {
  // Engine uses Workers fetch for DDG search — faster than Container's reqwest
  if (useEngine(env)) return engine.nativeSearch(env, options);
