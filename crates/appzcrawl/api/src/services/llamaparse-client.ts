...
export async function parsePdfWithLlamaParse(
  apiKey: string,
  pdfData: Uint8Array,
  opts: { timeoutMs?: number; wantsImages?: boolean } = {},
  ...
  try {
    ...
    if (wantsImages) {
      configuration.output_options = {
        images_to_save: ["embedded", "screenshot"] as const,
      };
    }
    ...
    if (!uploadRes.ok) {
      ...
      throw new Error(
        `LlamaParse upload failed: ${uploadRes.status} ${errBody}`,
      );
    }
    ...
    if (!jobId || typeof jobId !== "string") {
      throw new Error(
        `LlamaParse upload response missing job id: ${JSON.stringify(uploadJson)}`,
      );
    }

    // 2. Poll for completion
    for (let i = 0; i < MAX_POLL_ATTEMPTS; i++) {
      if (signal.aborted) throw new Error("LlamaParse aborted");

      const expand = wantsImages
        ? "markdown,images_content_metadata"
        : "markdown";
      const statusRes = await fetch(
        `${LLAMAPARSE_BASE}/parse/${jobId}?expand=${expand}`,
        {
          headers: { Authorization: `Bearer ${apiKey}` },
          signal,
        },
      ...
      if (!statusRes.ok) {
        ...
        throw new Error(
          `LlamaParse status failed: ${statusRes.status} ${errBody}`,
        );
      }
      ...
      const status =
        job.status ?? (statusJson as { status?: string }).status ?? "UNKNOWN";

      if (status === "COMPLETED") {
        const markdown =
          (statusJson as { markdown?: string }).markdown ??
          (job as { markdown?: string }).markdown ??
          "";
        ...
        const imgMeta = (
          statusJson as {
            images_content_metadata?: {
              images?: Array<{ presigned_url?: string }>;
            };
          }
        ).images_content_metadata;
      ...
      if (status === "FAILED" || status === "CANCELLED") {
        const errMsg =
          (job as { error_message?: string }).error_message ?? "Parse failed";
