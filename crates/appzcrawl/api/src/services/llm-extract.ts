...
export async function extractWithLlm(
  ...
  const messages: Array<{ role: string; content: string }> = [
    {
      role: "system",
      content:
        "You are a precise data extraction assistant. Output valid JSON only. Do not include markdown code fences or explanation.",
    },
  ...
  try {
    ...
    if (msg.includes("JSON Mode couldn't be met")) {
      logger.warn(
        "[llm-extract] JSON schema not satisfied, retrying without schema",
      );
...
function normalizeSchemaForWorkersAi(
  schema: Record<string, unknown>,
  ...
  if (!s.type && s.properties) {
    return {
      ...
      required: Array.isArray(s.required)
        ? s.required
        : Object.keys((s.properties as object) || {}),
      additionalProperties: false,
    } as Record<string, unknown>;
  }
