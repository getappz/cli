...
function evictOldestDomains(): void {
  if (domainLastRequest.size <= MAX_DOMAIN_ENTRIES) return;
...
async function throttleForDomain(url: string): Promise<void> {
  try {
...
export async function processScrapeQueue(
  batch: MessageBatch<ScrapeQueueMessage>,
  env: AppEnv["Bindings"],
): Promise<void> {
  const results = await Promise.allSettled(
    batch.messages.map((message) => processSingleMessage(message, env)),
  );
...
async function processSingleMessage(
  message: Message<ScrapeQueueMessage>,
  env: AppEnv["Bindings"],
): Promise<void> {
