...
const UNSUPPORTED_WARNINGS: Record<string, string> = {
 ...
 */
export function mapToFirecrawlResponse(
  ...
  if (
    ...
    warnings.push(
      "screenshot requires BROWSER_SERVICE binding (appz-browser) and valid screenshotBaseUrl",
    );
  }
  const warning =
    warnings.length > 0 ? warnings.join("; ") : (null as string | null);

  const data: ScrapeResponseData = {
    metadata: {
      ...document.metadata,
  ...
  const cachedAt =
    (document.metadata.cachedAt as string | undefined) ??
    new Date().toISOString();
  const cacheState =
    typeof document.metadata.cachedAt === "string" ? "hit" : "miss";
