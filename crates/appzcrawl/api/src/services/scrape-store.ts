...
export interface ScrapeRow {
  ...
  url: string;
  status: string;
  success: number; // D1 stores booleans as 0/1
  options: string | null;
  result: string | null;
  r2_key: string | null;
  error: string | null;
...
function rowToScrape(row: ScrapeRow): StoredScrape {
  return {
    id: row.id,
    teamId: row.team_id,
    url: row.url,
    status: row.status as StoredScrape["status"],
...
export async function createScrapeJob(
  ...
  await db
    .prepare(
      `INSERT INTO scrapes (id, team_id, url, status, success, options, zero_data_retention, created_at, updated_at)
       VALUES (?, ?, ?, 'pending', 0, ?, ?, ?, ?)`,
    )
    .bind(
      params.id,
 ...
 */
export async function getScrapeTeamId(
  db: D1Database,
  id: string,
 ...
 */
export async function updateScrapeSuccess(
  db: D1Database,
  id: string,
  result: ScrapeResponseData,
  r2Key?: string,
): Promise<void> {
  const now = new Date().toISOString();
  await db
 ...
 */
export async function updateScrapeFailure(
  db: D1Database,
  id: string,
  error: string,
): Promise<void> {
 ...
 */
export async function deleteOldScrapes(
  db: D1Database,
  olderThanMs: number,
): Promise<number> {
