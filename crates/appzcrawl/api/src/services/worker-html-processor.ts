...
async function drain(
  html: string,
  setup: (rw: HTMLRewriter) => HTMLRewriter,
): Promise<void> {
...
export async function extractBaseHref(
  ...
  await drain(html, (rw) =>
    rw.on("base[href]", {
      element(el) {
        if (baseHref) return; // only take first
        const href = el.getAttribute("href");
        if (href) {
          try {
  ...
  await drain(html, (rw) =>
    rw
      ...
      .on("title", {
        ...
        text(chunk) {
          if (capturingTitle) titleText += chunk.text;
          if (chunk.lastInTextNode) capturingTitle = false;
        },
      })
      // <meta …>
      .on("meta", {
        element(el) {
          ...
          if (name === "generator") metadata.generator = content;
          ...
          if (property === "og:locale") metadata.ogLocale = content;
          ...
          if (twitterKey) {
            ...
            (metadata.twitter as Record<string, string>)[key] = content;
          }

          // Dublin Core
          if (name.startsWith("dc.") || name.startsWith("dcterms.")) {
            if (!metadata.dublinCore) metadata.dublinCore = {};
            (metadata.dublinCore as Record<string, string>)[name] = content;
          }
        },
      })
      ...
      .on('link[rel="canonical"]', {
        element(el) {
          const href = el.getAttribute("href");
          if (href) metadata.canonical = href;
        },
      })
      // Favicon
      .on('link[rel="icon"], link[rel="shortcut icon"]', {
        element(el) {
          const href = el.getAttribute("href");
          if (href && !metadata.favicon) metadata.favicon = href;
        },
      }),
  );
...
export async function getInnerJson(html: string): Promise<{ content: string }> {
  let content = "";
...
export async function extractImages(
  ...
  if (baseResult.baseHref) effectiveBase = baseResult.baseHref;
...
export function postProcessMarkdown(
  ...
  for (const ch of markdown) {
    if (ch === "[") linkOpenCount++;
    else if (ch === "]") linkOpenCount = Math.max(0, linkOpenCount - 1);
 ...
 */
const EXCLUDE_NON_MAIN_SELECTORS: string[] = [
  "header",
  "footer",
  "nav",
  "aside",
 ...
 */
export function canHandleTransformHtml(params: TransformHtmlParams): boolean {
  // include_tags requires "keep only these" — needs DOM tree, not streaming
  if (params.include_tags && params.include_tags.length > 0) return false;
  // OMCE signature matching requires tree traversal with node signatures
  if (params.omce_signatures && params.omce_signatures.length > 0) return false;
  return true;
}
...
export async function transformHtml(
  ...
  try {
    ...
    if (baseResult.baseHref) baseUrl = baseResult.baseHref;
  } catch {
    // ignore
  }
  ...
  rw = rw.on("img", {
    element(el) {
      ...
      if (dataSrc) {
        ...
        if (resolved !== dataSrc) el.setAttribute("data-src", resolved);
      }
    },
  });
