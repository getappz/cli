...
/// Resolve relative URL against base (crawl4ai fast_urljoin logic).
...
// Matches ![alt](url) or [text](url) or [text](url "title")
static LINK_RE: Lazy<Regex> =
  Lazy::new(|| Regex::new(r#"!?\[([^\]]*)\]\(([^)]+?)(?:\s+"([^"]*)")?\)"#).expect("link regex"));

/// Convert markdown links to citations format (crawl4ai style).
/// `[text](url)` → `text⟨1⟩` with `## References` section at end.
