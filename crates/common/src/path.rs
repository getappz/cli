...
/// Check if a source path is at the root level
#[inline]
...
/// Standardize path separators to forward slashes (Unix-style)
#[inline]
pub fn standardize_separators<T: AsRef<str>>(path: T) -> String {
    path.as_ref().replace('\\', "/")
}

/// Source format for relative path expansion
...
/// Expand a path to workspace-relative format
...
/// Convert a path to a string, handling invalid UTF-8
#[inline]
...
/// Encode a value by removing invalid characters for use within a file name
...
/// Clean path components, removing redundant separators and resolving `.` and `..`
...
/// Check if two paths are equal after cleaning
