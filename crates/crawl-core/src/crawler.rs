...
#[cfg(feature = "native")]
pub async fn filter_links(data: FilterLinksCall) -> Result<FilterLinksResult, String> {
  let res = task::spawn_blocking(move || _filter_links(data))
    .await
    .map_err(|e| format!("filter_links join error: {e}"))?;

  res.map_err(|e| format!("Filter links error: {e}"))
}

/// Filter links based on crawling rules and constraints (sync version for WASM).
...
#[cfg(feature = "native")]
pub async fn filter_url(data: FilterUrlCall) -> Result<FilterUrlResult, String> {
  let res = task::spawn_blocking(move || _filter_url(data))
    .await
    .map_err(|e| format!("filter_url join error: {e}"))?;

  res.map_err(|e| format!("Filter URL error: {e}"))
}

/// Filter a single URL based on crawling rules and constraints (sync version for WASM).
#[cfg(not(feature = "native"))]
pub fn filter_url(data: FilterUrlCall) -> Result<FilterUrlResult, String> {
  _filter_url(data).map_err(|e| format!("Filter URL error: {e}"))
}
...
fn _parse_sitemap_xml(xml_content: &str) -> std::result::Result<ParsedSitemap, String> {
    ...
    match reader.read_event_into(&mut buf) {
      ...
      Ok(Event::End(e)) => {
        ...
        if local == b"loc" {
          ...
          if !loc.is_empty() {
            if root_name.as_deref() == Some(b"sitemapindex".as_ref()) {
              sitemaps.push(SitemapEntry { loc: vec![loc] });
            } else {
              urls.push(SitemapUrl { loc: vec![loc] });
            }
          }
          current_loc.clear();
        }
  ...
  match root_name.as_deref() {
    Some(b"sitemapindex") => Ok(ParsedSitemap {
      urlset: None,
      sitemapindex: Some(SitemapIndex { sitemap: sitemaps }),
    }),
    Some(b"urlset") => Ok(ParsedSitemap {
...
#[cfg(feature = "native")]
pub async fn parse_sitemap_xml(xml_content: String) -> Result<ParsedSitemap, String> {
  let res = task::spawn_blocking(move || _parse_sitemap_xml(&xml_content))
    .await
    .map_err(|e| format!("parse_sitemap_xml join error: {e}"))?;

  res.map_err(|e| format!("Parse sitemap XML error: {e}"))
}

/// Parse XML sitemap content into structured data (sync version for WASM).
#[cfg(not(feature = "native"))]
pub fn parse_sitemap_xml(xml_content: String) -> Result<ParsedSitemap, String> {
  _parse_sitemap_xml(&xml_content).map_err(|e| format!("Parse sitemap XML error: {e}"))
}
...
#[cfg(feature = "native")]
pub async fn process_sitemap(xml_content: String) -> Result<SitemapProcessingResult, String> {
  let res = task::spawn_blocking(move || _process_sitemap(&xml_content))
    .await
    .map_err(|e| format!("process_sitemap join error: {e}"))?;

  res.map_err(|e| format!("Parse sitemap error: {e}"))
}

/// Process sitemap XML and extract crawling instructions (sync version for WASM).
#[cfg(not(feature = "native"))]
pub fn process_sitemap(xml_content: String) -> Result<SitemapProcessingResult, String> {
  _process_sitemap(&xml_content).map_err(|e| format!("Parse sitemap error: {e}"))
}
