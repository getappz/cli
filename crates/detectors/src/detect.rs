...
use crate::languages::detect_languages;
use crate::testing::detect_testing;
use crate::tools::detect_tools;
use ::frameworks::detector_types::{DetectorStatic, Framework};
...
/// Result of full project detection for skills recommendations.
#[derive(Debug, Clone)]
pub struct SkillsDetectionResult {
...
/// Detect all project characteristics for skills recommendations.
/// Returns frameworks (slugs), languages, tools, testing, and combined search_terms.
pub async fn detect_for_skills(
    fs: &Arc<dyn DetectorFilesystem>,
    all_dependencies: &HashMap<String, String>,
) -> Result<SkillsDetectionResult, String> {
    ...
    let frameworks = detect_frameworks(options)
        .await?
        .into_iter()
        .filter_map(|f| f.slug.map(String::from))
        .collect::<Vec<_>>();

    let package_manager = detect_package_manager(fs)
        .await?
        .map(|pm| pm.manager);

    let languages = detect_languages(fs, all_dependencies).await;
    let tools = detect_tools(fs, all_dependencies).await;
    let testing = detect_testing(fs, all_dependencies).await;

    let mut search_terms: Vec<String> = frameworks
        .iter()
    ...
    search_terms.sort();
    search_terms.dedup_by_key(|s| s.to_lowercase());

    Ok(SkillsDetectionResult {
        package_manager,
        frameworks,
        languages,
        tools,
