...
impl GlobalEnvBag {
        ...
        K: AsRef<OsStr>,
    {
        ...
        if let Some(value) = env::var_os(key) {
            ...
            let _ = self.inherited.upsert_sync(key.into(), value);
    ...
    pub fn should_debug_wasm(&self) -> bool {
        self.get_as("APPZ_DEBUG_WASM", as_bool).unwrap_or_default()
    }

    pub fn should_debug_mcp(&self) -> bool {
        self.get_as("APPZ_DEBUG_MCP", as_bool).unwrap_or_default()
    }

    pub fn should_debug_process_env(&self) -> bool {
        self.get_as("APPZ_DEBUG_PROCESS_ENV", as_bool)
            .unwrap_or_default()
    }

    pub fn should_debug_process_input(&self) -> bool {
        self.get_as("APPZ_DEBUG_PROCESS_INPUT", as_bool)
            .unwrap_or_default()
    }

    pub fn should_debug_remote(&self) -> bool {
        self.get_as("APPZ_DEBUG_REMOTE", as_bool)
            .unwrap_or_default()
    }
}
