#!/bin/sh
# install.sh — Install appz (download pre-built binary or build from source)
#
# Usage:
...
print_banner() {
  ...
  case "${APPZ_QUIET_INSTALL:-}" in 1|true) return 0 ;; esac
  ...
  printf '%s\n' "${CY}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RS}"
  printf '%s\n' "${MG}  appz  ·  Detect toolchains, wire up mise, run install/build/dev/test${RS}"
  printf '%s\n' "${CY}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RS}"
}

REPO="getappz/cli"
INSTALL_DIR="${APPZ_INSTALL_DIR:-$HOME/.local/bin}"
...
finish() {
  ...
  echo ""
  echo "Done! Verify with: appz --version"
  echo ""
  echo "Next step: appz init"
}
...
install_download() {
  ...
  if ! curl -fsSL "$asset_url" -o "$tmpdir/appz.tar.gz"; then
  ...
  # what a selective MITM wants. APPZ_SKIP_VERIFY=1 is the escape hatch.
  if curl -fsSL "$sums_url" -o "$tmpdir/SHA256SUMS" 2>/dev/null; then
    expected="$(grep "appz-${target}.tar.gz" "$tmpdir/SHA256SUMS" | cut -d' ' -f1)"
    if [ -n "$expected" ]; then
      verify_checksum "$tmpdir/appz.tar.gz" "$expected"
    elif [ "${APPZ_SKIP_VERIFY:-0}" = "1" ]; then
      echo "  Warning: appz-${target}.tar.gz not listed in SHA256SUMS — proceeding unverified (APPZ_SKIP_VERIFY=1)"
    else
      echo "Error: appz-${target}.tar.gz is not listed in SHA256SUMS — refusing to install unverified."
      echo "Set APPZ_SKIP_VERIFY=1 to bypass (not recommended)."
      exit 1
    fi
  elif [ "${APPZ_SKIP_VERIFY:-0}" = "1" ]; then
    echo "  Warning: checksums not available — proceeding unverified (APPZ_SKIP_VERIFY=1)"
  else
  ...
  tar -xzf "$tmpdir/appz.tar.gz" -C "$tmpdir"

  mkdir -p "$INSTALL_DIR"
  tmp_bin="$INSTALL_DIR/.appz.new.$$"
  install -m755 "$tmpdir/appz" "$tmp_bin"

  if [ "$(uname -s)" = "Darwin" ]; then
    xattr -cr "$tmp_bin" 2>/dev/null || true
    codesign --force --sign - "$tmp_bin" 2>/dev/null || true
  fi
  mv -f "$tmp_bin" "$INSTALL_DIR/appz"
  tmp_bin=""

  echo "  Installed: $INSTALL_DIR/appz"
...
install_from_source() {
  ...
  echo ""
  echo "Building appz (release)..."

  (cd "$SCRIPT_DIR" && cargo build --release -p appz)
  target_dir=$( (cd "$SCRIPT_DIR" && cargo metadata --no-deps --format-version=1 2>/dev/null) \
      ...
      | sed 's/\\\\/\//g' || true)
  binary="${target_dir:-$SCRIPT_DIR/target}/release/appz"
  ...
  mkdir -p "$INSTALL_DIR"
  tmp_link="$INSTALL_DIR/.appz.link.$$"
  ln -sf "$binary" "$tmp_link"
  mv -f "$tmp_link" "$INSTALL_DIR/appz"
  echo "  Linked: $INSTALL_DIR/appz -> $binary"
...
uninstall() {
  echo "Mode: uninstall"
  echo ""
  for b in "$INSTALL_DIR/appz" "/usr/local/bin/appz"; do
    if [ -e "$b" ] || [ -L "$b" ]; then
      rm -f "$b" 2>/dev/null && echo "  Removed $b" || true
    fi
  done
  echo ""
  echo "appz binary removed. Project state under .appz/ and any mise.toml appz"
  echo "wrote are untouched — remove those yourself if you no longer want them."
  echo "Verify with: command -v appz   # should print nothing"
}
...
case "${1:-}" in
  ...
  --help|-h)
    ...
    echo ""
    echo "  (no args)     Build from source if run inside an appz checkout, else download"
