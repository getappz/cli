...
dist_tag="$(dist_tag_from_version "$APPZ_VERSION")"

# Strip leading 'v' for npm (npm versions don't allow 'v' prefix)
APPZ_NPM_VERSION="${APPZ_VERSION#v}"
...
download_asset() {
	...
	rm -f "$dest"
	gh release download "$APPZ_VERSION" \
		--repo getappz/cli \
		--pattern "$asset" \
		--dir "$RELEASE_DIR" || {
...
extract_asset() {
	...
	*)
	...
	if [ -f "$dest_dir/appz" ] || [ -f "$dest_dir/appz.exe" ]; then
		mkdir -p "$dest_dir/bin"
		mv "$dest_dir"/appz* "$dest_dir/bin/" 2>/dev/null || true
	fi
}

skipped_platforms=()
published_platform_deps=() # "@scope/appz-os-arch" entries actually published, for the main package's optionalDependencies

for entry in "${platforms[@]}"; do
	IFS=":" read -r npm_plat rust_target ext <<<"$entry"
	IFS="-" read -r os arch <<<"$npm_plat"

	asset="appz-${rust_target}.${ext}"
...
EOF
	...
	if [ "${DRY_RUN:-0}" == 1 ]; then
		echo "DRY RUN: would publish $pkg_name@$APPZ_NPM_VERSION"
	else
		npm publish --access public --tag "$dist_tag" --provenance || {
			if npm view "$pkg_name@$APPZ_NPM_VERSION" version &>/dev/null; then
				echo "Version $APPZ_NPM_VERSION already published for $pkg_name, skipping"
			else
...
# every other dependency), and bin/appz.js is a small shim that, at
...
# bin/appz.js -- embedded to keep it versioned with the script
cat <<'JSEOF' >"$RELEASE_DIR/npm/bin/appz.js"
#!/usr/bin/env node
"use strict";
...
var PLATFORM_PACKAGES = {
    "linux-x64": "@getappz/appz-linux-x64",
    "linux-arm64": "@getappz/appz-linux-arm64",
    "darwin-x64": "@getappz/appz-darwin-x64",
    "darwin-arm64": "@getappz/appz-darwin-arm64",
    "win32-x64": "@getappz/appz-win32-x64"
};
...
function resolveBinary() {
    ...
    if (!pkg) {
        console.error("appz: unsupported platform " + key);
        process.exit(1);
    }
    ...
    try {
        ...
        console.error(
            "appz: platform package " + pkg + " is not installed.\n" +
    ...
    var binRel = typeof pkgJson.bin === "string" ? pkgJson.bin : pkgJson.bin.appz;
...
if (result.error) {
    console.error("appz: failed to launch " + bin + ": " + result.error.message);
    process.exit(1);
}
if (result.signal) {
...
JSEOF
chmod +x "$RELEASE_DIR/npm/bin/appz.js"
...
if [ "${#published_platform_deps[@]}" -gt 0 ]; then
	...
	for dep in "${published_platform_deps[@]}"; do
		optional_deps_entries+="    \"$dep\": \"$APPZ_NPM_VERSION\",\n"
	done
	optional_deps_json="{\n${optional_deps_entries%',\n'}\n  }"
fi
...
pushd "$RELEASE_DIR/npm"
if [ "${DRY_RUN:-0}" == 1 ]; then
	echo "DRY RUN: would publish ${NPM_MAIN}@$APPZ_NPM_VERSION"
else
	npm publish --access public --tag "$dist_tag" --provenance || {
		if npm view "${NPM_MAIN}@$APPZ_NPM_VERSION" version &>/dev/null; then
			echo "Version $APPZ_NPM_VERSION already published, skipping"
		else
...
if [ "${#skipped_platforms[@]}" -gt 0 ]; then
	error "npm publish incomplete for v$APPZ_NPM_VERSION — missing platform assets: ${skipped_platforms[*]}"
fi

echo "npm publish complete for v$APPZ_NPM_VERSION"
